...
#   🔒 `ap`〜`ar` 係 `W-G.9-270` 補令二 `§六`（**全數核可**）所立（本波末三坑）。
#      ⚠️ **本清單係<u>硬編</u>之母體**——新坑一經核可，須**同批**擴之；
#      否則該器**結構上看不見**新坑，而其輸出仍為「`67`／`67` 全數可解析」⇒ **靜默漏**
...
ALPHA_LABELS = ([chr(c) for c in range(ord("a"), ord("z") + 1)]
                + ["aa", "ab", "ac", "ad", "ae", "af", "ag", "ah", "ai", "aj", "ak",
                   ...
                   "bd",              # 🆕 `W-G.9-278` §三 所准之一坑
                   "be"])             # 🆕 `W-G.9-279` §三 所准之一坑
...
def main():
    ...
    for rev in revs:
        ...
        run(rev, ALPHA_LABELS, "字母系 `a`〜`z` ＋ `aa`〜`be`")
        issuer_frame(rev, NUM_LABELS, "數字系 `1`〜`26`")
        issuer_frame(rev, ALPHA_LABELS, "字母系 `a`〜`z` ＋ `aa`〜`be`")
    return rc
