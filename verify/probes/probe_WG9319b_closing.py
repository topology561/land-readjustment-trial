# -*- coding: utf-8 -*-
r"""W-G.9-318 W-G.9-319 `§四`：收工閘 `1`〜`8` 之實測（報告入倉後量·唯讀）。

🔒 開工態 ＝ `549121b…`（`恆常附款 w②`·⛔ 沿用他批）。
🔒 閘 `4` 之 `GB` 依W-G.9-319 `裁 2`：以 `probe_WG9267_issuer_measurers.py registry` 之
...
BASE = "51a641d334847d285f323257909748acdf7052e1"
...
def main():
    ...
    print("# `W-G.9-319 補令一`　收工閘（報告入倉後量）")
    ...
    print("  🔒 自誤之算式（`w①`）：開工 `435` ＋ 本批新鑄 `3`（`自誤 446〜448`）＝ **`438`**；MAX ＝ **`448`**")
    print("  🛑 `GB` 依補令五 `裁 2` 以 `probe_WG9267_issuer_measurers.py registry` 之**自擬框**出艙")
    ...
    # 🩸 單 `§六` 閘 `6` 之期值逐字為主線 `4037570…`——該值係**上一批快轉<u>前</u>之態**，
    #    於本批開工態（`c286a79`）已過期。依 `恆常附款 f`（不符者以**列舉**為準並具名）
    #    改以**本批開工態**為期值，並**二值並列**；⛔ 追改單一字。
    print("  主線 ＝ `%s`" % m[0])
    print("     單載期值 ＝ `%s…`（🩸 **已過期**·係上批快轉前之態）／"
          "本批開工態 ＝ `%s…` ⇒ %s"
          % (MAIN[:7], BASE[:7], "🟢 **與開工態相符·未動**" if m[0] == BASE else "🔴 **已被他動**"))
    print("     `is-ancestor 4037570 → 主線` ⇒ `rc` `%d`（須 `0`·證其仍在祖先鏈內）\n"
          % g(["merge-base", "--is-ancestor", MAIN, m[0]]).returncode)
