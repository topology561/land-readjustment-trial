# -*- coding: utf-8 -*-
r"""W-G.9-318 補令五 `§四`：收工閘 `1`〜`8` 之實測（報告入倉後量·唯讀）。

🔒 開工態 ＝ `549121b…`（`恆常附款 w②`·⛔ 沿用他批）。
🔒 閘 `4` 之 `GB` 依補令五 `裁 2`：以 `probe_WG9267_issuer_measurers.py registry` 之
...
BASE = "549121b27beb6a34b3a6c823af43c78e37b0c7ee"
...
def main():
    ...
    print("# `W-G.9-318` 補令五　收工閘（報告入倉後量）")
    ...
    print("  🔒 自誤之算式（`w①`）：開工 `432` ＋ 本批新鑄 `1`（`自誤 443`）＝ **`433`**；MAX ＝ **`443`**")
