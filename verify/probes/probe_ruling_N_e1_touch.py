...
def _e7_merge_gate(ns, g_rows, front_lines, min_depth_by_blk, min_width_by_blk,
    ...
    **本閘尚未改**（實作屬 **K-6-A2**）⇒ 本 docstring 所述現況為**過渡態**。
    ⛔ 本檔 `_measure` 內之 `判定` 欄字面**綁 baseline 13 列**（0m 8／3.5m 5）
       ⇒ **本批不動**（改字串＝重烤假紅）。遷移項登記見上開正典之
       「K-8 波末遷移項登記」節。
...
def _measure(g_rows, front_lines, min_depth_by_blk, width_fn, tag):
    ...
    for r in g_rows:
        ...
        if is_corner:
            ...
            #    `grep -n "^## K-9 " docs/rulings/K-6_街角地分配程序與可分配判準.md`）：
            #    街角第 1 宗改**寬度實量**（截角前），門檻＝退縮寬＋畸零地最小寬。
            #    ⚠️ **本探針尚未改**（實作屬 K-6-A2）⇒ 下行為**過渡態**。
            #    ⛔ 下行之字面**綁 baseline 13 列**（`判定` 欄）⇒ **本批不動**。
            #    📐 八街角實測寬度另見 `probes/probe_ruling_K9_corner_width.py`（只量不判）。
