...
🗄️ **本檔所回答之問題已於 2026-08-05 由 KL 圖示裁定作廢·原檔存查勿刪**（K-6-A2 段五(c)-1）。
   **量測對象不改**（量宗地多邊形之弦是對的）；錯的是**帶底線之位置**
   ——`K-9-6-a` 表列第 1 列之受詞與法規原文相反。教訓：**失敗考古 節 42**。
   ⚠️ 本檔仍可跑（`_n14_band_hi` 之五元組形未變），惟其「正典×新帶」欄
   **係以 (c)-1 前之舊帶底算出**；(c)-1 後重跑之數字**不具原本之對照意義**。
   現行正解見 `verify/probes/probe_K9_6_parcel_band.py` 與
   `docs/reports/W-G.6_K6-A2-段五c1_宗地帶_攤表.md`。
