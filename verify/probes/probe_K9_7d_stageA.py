...
⇒ 立檔時**生產碼一字未改、第二層為零生產呼叫點**。
🔴 **K-6-A2 段四(c) 起生產已切換為第二層** ⇒ 本檔之注入對「二層」已成同值之 no-op；
  本檔之價值改為**保留階段 A／B 之可重跑對照**（`delta_p=0` 重現甲式帶）。
...
def main():
    ...
    L = []
    L.append("=" * 132)
    L.append("【K-9-6-b-2 乙式·階段 A／B】甲式帶 vs 乙式帶 —— "
             "立於 K-6-A2 段四(c)-2(a′)；**段四(c) 起生產已為乙式**")
    ...
    L.append("")
    L.append("【D】註記")
    L.append("  · 🔴 **段四(c) 起生產已走第二層** ⇒ 本檔之 `_t_override` 注入已成同值 no-op；"
             "patch 僅存在於本行程、生產碼一字未改。")
