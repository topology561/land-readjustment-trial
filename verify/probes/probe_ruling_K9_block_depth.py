...
def main():
    ...
    # ⚠️ **射程更正（`W-G.9-42` 加註·⛔ 上行字串一字不刪·⛔ 零行為變更）**：上行之
    #   「一律不量深度」**過寬**，正確為「**量、但不比附表深度**」——`K-9-3-1`（KL 裁 2026-08-05）。
    #   ⛔ 不重述，錨 `grep -n "^### 🔒 K-9-3-1 " docs/rulings/K-6_街角地分配程序與可分配判準.md`。
    #   🔒 **⛔ 刻意不改上行之字串值、⛔ 亦不新增任何 `L.append`**：本探針之輸出係
    #      **歷史 log 之對拍面**（`verify/out/probe_ruling_K9_block_depth.log`），
    #      改字串**或多印一行**皆改受詞（`CLAUDE.md`「凍存受詞」族）
    #      ⇒ 更正一律以**本註**承載。本批對本檔之異動為 **`+5 / −0` 且全為註解行**。
