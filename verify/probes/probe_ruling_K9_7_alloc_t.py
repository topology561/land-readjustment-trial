...
def main():
    ...
    # ── 🗄️ 【D】舊「`P_front` 落截角旗標」段已刪（GB-37 已收·K-6-A2 段四(c)）────────
    L.append("")
    L.append("【D】🗄️ **本段已刪**（GB-37 已收·K-6-A2 段四(c)）")
    L.append("  舊段以 **`P_front(t*)`**（`ALLOC ∩ 前緣線`）測「落截角三角形內」，"
             "對 GB-19 之觸發條件係**假陰性**：")
    L.append("  實測 `P_front` **0/16**，而 K-9-8 (2) 之 **`P_block` 4/16 離開 FRONT**。")
    L.append("  ⇒ 受測對象自始就錯（`P_front` 與 `P_block` 同名異物·K-9-7-a 衝突二）；"
             "旗標與其回傳鍵已整段移除。")
    L.append("  🔒 **`P_block` 之落點表請看** "
             "`verify/out/probe_K9_7d_layer2.log`【C】段"
             "（探針 `verify/probes/probe_K9_7d_layer2.py`）。")
    L.append("  ⛔ **本段不改為讀 `P_block_on_chamfer`**——本檔**不傳** `block_vertices`，"
             "取到必為 `None`，那只會生一個新的假陰性。")
    L.append("")
    L.append("【E】註記")
