...
# 🗄️ ─── 以下 F-3 註解塊描述 **K-6 前**之狀態，**已由下方 K-6 註解塊取代** ───────────
#   · `:60` 之「3.5m 少一對」**已不成立**——K-6 §四作廢 M-5 後該對已補回，兩情境同構。
#   · 「🚩 波次歸屬待裁」旗**因 M-5 作廢而消滅**（不再有「同一機制擾動三錨」之情形）。
#   · **保留不刪之理由**：其中「M-5 把 `628-45(1)`／`628-30(1)` 改道、溶掉 G007 GSA 錨」
#     之觀察係 K-6-A1 得以定位 12 長期紅之關鍵線索，具考古價值。
# 🆕 F-3（裁定M·claude.ai 2026-07-25）：**per-tag**——3.5m 少 `628-30(2)→628-45(2)` 一對，
#   係 **M-5 已於 PK 階段提前完成同一對合併**（628-30(2) 為 M-5 ①之出資源、628-45(2) 為街角
#   winner／受贈 target——原就是 E0 之既有配對）⇒ **就本 dict 所移除之該一鍵而言**係
#   **階段位移**（E0→PK）：同一對地仍合併、僅時點提前，故 3.5m 之 E0 階段不再重複配對。
#   ⚠️ **範圍限定**（reviewer W-4）：此「非行為改變」**僅指本鍵**。M-5 同時把 `628-45(1)`／
#   `628-30(1)` 自其 F.0-級0 歸宿（target `628-20(2)@R5`）**改道**至 `628-45(2)@R3`——
#   **換 target／換街廓／換世代**，該部分**係行為改變**（亦即溶掉 G007 GSA 錨之主因），勿以本註解泛稱。
#   0m 無 M-5 award ⇒ 三對不變。
#   🚩 **波次歸屬待裁**：同一 M-5 機制擾動 GSA／k*／E0 三錨，前二者留紅待 P-H、本錨於 F-3 先行重錨
#   ——不對稱，已上呈 KL/claude.ai（見 F-1~F-5 完成報告 §九）。
# 🆕 **K-6（KL 裁 2026-07-30）：3.5m 補回 `628-30(2)→628-45(2)`**───────────────────
#   舊 3.5m 錨缺該對之成因 ＝ **M-5 街角救援**（K-4-3）在 F.0 看見 `628-30(2)` 之前
#   已先把它當來源消費掉（其 target 正是 `628-45(2)`＝M-5 唯一 award 之 winner）。
#   **自證**：`0m` 錨本來就有該對——0m 無 forced 端 ⇒ M-5 從不執行 ⇒ 未被攔截。
#   ⇒ K-6 §四 作廢 M-5 後，該同歸戶集中**改於 E0（級0）發生**，兩情境回復同構。
#   這正是 K-6 §三「跨街廓同歸戶『小往較大集中』一律歸七級調配（F.0／F.2）；
#   **街角不再有專屬救援機制**」之預期結果，**非**回歸破口。
E0_EXPECT = {
    "0m":   {"628-49(1)": "628-48(1)", "628-30(2)": "628-45(2)", "628-42(1)": "628-42(2)"},
    "3.5m": {"628-49(1)": "628-48(1)", "628-30(2)": "628-45(2)", "628-42(1)": "628-42(2)"},
}
