...
## W-D.4 四梯分級清單（清單波，2026-07-08）

- 產出：`verify/wd4_tier_list.py`（committed）雙情境；三 CSV 轉正：`W-D.4_四梯分級清單`／
  `W-D.4_碎片遞補對照`／`W-D.4_跨占分配線`_退縮{0m,3.5m}。
- **歸戶主體＝Gxxx 指紋群**（KL 撤銷改判 2026-07-08；ΣG_戶＝群內 G 直加、四梯走群組）。
- **零幾何移動零財務實算**：run_all 既有 20 閘對凍結 baseline 逐格不變（G/守恆/位次/J/k* 未動）。
- **參考估算非實算聲明**：清單之 增配/差額/放棄/補償 欄一律帶「(參考,a′待v3)」，**乘 Tab4
  後地價僅供標註、永不進 G/守恆式**（裁定二圍欄）；a′/增配/補償實算待 v3 財務接線。
- 錨：MinA_區=114.065(R4·未 round)／½線 57.0325；22 旗標全消費（群組語意）；R6 85.66→628-1(2)
  跳過 628(2)；查封 fixture 自成一群·阻卻=是·移除後 byte-identical。
- 護欄：WARNING-A 持分和**逐宗**檢核（分別共有 Σ≈1.0）；WARNING-B MinA 斷言用**未 round 原積**
  容差<0.01。碎片遞補之三分類源＝W-D.3 `wd3_fragment_geom`（單一真相源）。
