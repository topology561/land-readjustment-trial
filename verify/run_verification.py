...
_DECLARED_EXTRAS = {
    ...
    "W-D.1.2 診斷_退縮0m.csv": {
        ...
        "指數名次": "K-6-B 段二鍵序① 之來源（`W-G.9-198R` `R1-3`）；該側 group 全體依 "
                    "`priority_index` 降冪之名次·baseline 烤於其之前（2026-09-02 宣告）",
    },
    "W-D.1.2 診斷_退縮3.5m.csv": {
        "真G(㎡)": "同上（2026-07-27 宣告）",
        "指數名次": "同上（2026-09-02 宣告）",
    },
}
...
def main():
    ...
    for setback, tag in ((0.0, "0m"), (3.5, "3.5m")):
        ...
        #   **街角不再有專屬救援機制**。碼移至 `verify/archive/m_rescue.py`（禁再接線）。
        #   ⇒ 本迴圈自此**只跑趟0 PK**（無物化、無趟1 重跑）；`parcels₁` 隨之消滅，
        #     下游 `run_step_g`／F.0 ctx 一律吃 `build_parcels`（**不留 `.get(tag, …)` fallback**）。
    ...
    try:
        ...
        # 🆕 F-7-6(b)：標題**隨 tag 現算對數**。**設計理由不變**：對數係 per-tag 之導出量，
        #   寫死即在機制變動時默默過期。⚠️ 舊註解稱「3.5m 經 M-5 提前合併後為**兩對**」
        #   ——**已過期**：K-6 §四作廢 M-5 後 3.5m 回到**三對**（與 0m 同構）。
        #   此正是「隨 tag 現算」之價值：碼未動而數字自動跟上。
