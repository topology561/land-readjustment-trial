...
- ✅ **P6 Step G headless（W-D.2 v2 轉正，2026-07-06）**：`stepg_pipeline.py` 消費快照
  `負擔尺度_輸入`（條件①斷言）＋`StepG_v2_中間態`（C=0/A=1，Tab7/Tab5 未跑中間態）→
  G 值計算結果／滑池槽診斷／逐槽 J 表 三表雙情境 cell-exact 對拍 `baselines/v2/`
  （KL 2026-07-06 準源輪；PROVENANCE_v2.md）。J 非零看守（條件③）＝有 SIDE_LINE 側
  F×l₁≤0 即 RuntimeError。**[H-ghost] 假說上呈中**：harness 排除 `_is_ghost_sliver`
  以重現 KL 輪（詳 PROVENANCE_v2 未決事項）。
