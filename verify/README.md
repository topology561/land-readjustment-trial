...
3. ~~skip_col『原位次(距角序·暫行)』（v1 W-D.1.2 診斷）~~ → **2026-07-10 除籍**：
   W-D.1.2 診斷已重烤為 `baselines/v3/`（harness 產、欄名『原位次(投影序)』），
   **主閘改全欄逐格對拍、零豁免**（閘反而收緊）。
   唯一殘存 skip 在 **「率接線無串聯」差分證明閘**：`got 診斷 vs v1 原錨`
   豁免 `{原位次(距角序·暫行), G估(㎡)}` 後**必須逐格全等**——此非「遮真 diverge」，
   而是**刻意差分**：專門證明「重劃總負擔率接線只動 G估 欄，達標/選中/總分/門檻/
   真交集/範圍/評分全未動」。G估 之變動量另由「G估 欄變動 18/21 格」錨看守，
   winner 之不變由 v1 指配表／抵費地表逐格對拍看守。**兩者皆為加閘、非鬆閘。**
新增任何 norm/skip 規則**必須在此列名＋說明「為何這不是在遮真 diverge」**。
...
2. **driver orchestration 漂移**：未來任何波動到**街角 callback**（`app.py` ~13765-14107）或
   **tab1 歸戶 inline**（~10844-11049）→ **必須同波更新 `verify/selection_pipeline.py`**
   ...
   - 記帳（W-D.2，2026-07-05）：PK callback 結果彙總段新增 `_forced_offset_map` 之
     `left/right_corner_min_area` 鍵（守恆接線餵入端）——**三張 CSV 不消費此段**、
     selection_pipeline 不複製；run_all 11/11 綠＝未擾 §1 之證。Step G（推進/滑池槽）
     非 harness 現覆蓋範圍；其 headless 化＝W-D.2 P6（baselines v2 轉正必要條件）。
...
- ✅ **Step G headless（v3 財務接線波轉正，2026-07-10）**：`stepg_pipeline.py` 消費快照
  `負擔尺度_輸入`（條件①斷言）＋`財務接線_v3`（**真值 A 逐宗／B／C**）→ G 值計算結果／
  滑池槽診斷／逐槽 J 表 三表雙情境 cell-exact 對拍 `baselines/v3/`（PROVENANCE_v3.md）。
  - **廢止**：`StepG_v2_中間態`（C=0/A=1 半中間態）、`財務接線_全區_步驟F`、
    `財務接線_重劃後街廓地價_Tab4`、`財務接線_重劃前地價區段_Tab5` 四節整刪、無 fallback。
    `baselines/v2/` **凍存不刪、不再對拍**（半中間態期之歷史錨）。
  - **財務閘**：B==0.171043／C==0.091319（由三獨立純量現算，**禁反填**）＋中繼錨
    ＋C 兩形等價（加總形 vs `calc_C_value` 均價形，|Δ|<1e-8）＋A 逐宗全查（118 列）
    ＋**雙 reverse-test 真引擎重跑**（改單位工程費→C 隨動；改前均價→B 隨動且 C 不動）。
  - **結構不變量永久閘**（`run_step_g` 內 RuntimeError）：守恆／⊥（ALLOC⊥FRONT）／
    位次＝投影序／J(k\*)≥J(naive)／理論＝實跑／telescoping（ΣRw_側 = R(W_final)−R(W₀_buffer)）。
    **取代**舊「J/ΣRw 逐格不變」閘——後者僅成立於 Rw 飽和域（PROVENANCE_v3 註①）。
    `k*` 為**經驗**不變量（非機制保證），由 `K_STAR_EXPECT` 硬斷言看守。
  - **重劃總負擔率現算**（微波 2026-07-10）：`compute_total_burden_rate()` ＝
    公設負擔比(DXF 公設÷總面積精確) ＋ 費用負擔比(費用總額÷[後均價×總面積精確]) = **0.40712387**。
    廢 `global.重劃總負擔率` 寫死鍵與 `or 0.40` fallback（缺值 loud）。餵 G估 鏈
    （`_estimate_G_for_qualification`）＋ `iterate_G_S` 初值。reverse-test③ 改單位工程費→率隨動。
    W-D.1.2 診斷隨之重烤 v3；**無串聯機器證明**：vs v1 原錨豁免 G估 後逐格全等、
    G估 變動 18/21 格、winner/指配/抵費地 vs v1 逐格不變。
  - J 非零看守（條件③）＝有 SIDE_LINE 側 F×l₁≤0 即 RuntimeError。
    [H-ghost] 已決（consistent、不升版；見 PROVENANCE_v2）。
  🔧 耦合：app Step G orchestration（推進/滑池槽/抵費地/ledger）波動 → 必同波更新
  `stepg_pipeline.py`（逐行複刻段）。
...
- `build_ownership()`＝**逐行複刻** app.py tab1 歸戶 inline（~10800-10993；st.button 分支內
  ...
  變體鍵）＋ `t8_parcel_areas`。
- `build_build_parcels(ns, fake_st, v6_bytes, cb, snapshot)`＝真函式管線：`parse_cadastral_geofile` →
  `validate_parcel_assignments_by_area(0.30, 30m)` → `overlay_polygons_to_blocks` →
  `_assign_four_column_areas` → `_annotate_temp_parcel_cut_type` → 🆕 `_attach_pre_zone`
  → 可建築過濾。**`snapshot` 為必填**（v3 起 A 逐宗查價需 `財務接線_v3.原地號_區段`；
  漏傳即 TypeError，不容靜默略過 zone）。非-ghost 原地號不在 53 筆區段表 → loud RuntimeError
  （禁靜默退回 A=1）；ghost sliver（四欄面積恆 0、Step G 前即過濾）zone=''、不 raise。
  呼叫端四處：`run_verification` / `wd3_fragment_geom` / `wd4_tier_list` / `b6_isomorphism`。
