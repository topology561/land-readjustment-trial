...
def export_legal_excel(g_rows: list, ownership_map: dict,
    ...
    **非** T2 正確性之證明（後者靠 ②不變量閘＋③預測差量閘）。
    ...
    #      空白／None 代表未決者一律禁止。
    #   🔴 **K-9-5 已撤銷本豁免**（K-7 作廢·KL 裁 2026-08-01）：街角第 1 宗**寬度要實量**
    #      （**截角前**幾何），門檻＝退縮寬＋畸零地最小寬。現行碼**尚未實作實量**，
    #      實作屬 **K-6-A2** ⇒ **本註解描述之現況為過渡態**。
    ...
    for r in (g_rows or []):
        ...
        ws1.cell(row=row, column=13, value=width_verdict_of(r))
        row += 1
    ...
    if _offsets:
        ...
        for r in _offsets:
            ...
            ws1.cell(row=row, column=13, value=WIDTH_VERDICT_NA)
            row += 1
    # 欄寬
    for col_letter, width in zip('ABCDEFGHIJKLM',
                                 [8, 30, 16, 14, 12, 14, 30, 14, 14, 14, 12, 12, 16]):
...
def _compute_block_depth_alloc(block_vertices, block_area, alloc_dir,
    ...
    **🆕 K-9-2 剖面欄（`d_front_p1`／`d_front_p2`／`D_k92_min`·全精度·未 round）**：
      前緣線 p1（左）／p2（右）兩端至 BASELINE **無限直線**之垂距本身（＝ `d1`／`d2`），
      及其 **min**。K-9-2 判準＝「前緣線上**任一點** ≤ 畸零地最小深度即警示」；
      垂距沿弦**線性** ⇒ **min 必落於兩端之一** ⇒ 取 `min(d1, d2)`。**解析式·禁取樣、禁掃描。**
      （消費端＝`k92_block_depth_check`：`grep -n "def k92_block_depth_check" app.py`）
      ⚠️ **`D_k92_min` ≠ `D_min`**（**同名不同量·嚴禁互代**·比照 K-9-6-a 之戒）：
        · `D_min` ＝ **方法 A 診斷**（沿 `alloc_dir` 剖面差值·**取樣**）之極小值；
          僅於方法 A 不可算時才退為 `min(d1,d2)`（見 `_src`／`note`）⇒ **軸與來源皆可能不同**。
        · `D_k92_min` ＝ **恆為** N-19′ 軸（垂直 BASELINE）之兩端垂距最小值、**恆不取樣**。
        ⇒ **K-9-2 一律取 `D_k92_min`，不得改讀 `D_min`。**
      🔒 **本三欄不 round**：門檻比較用**未 round 之原值**、`round()` 只在輸出層
        （K-8 §二-1 WARNING-B 體例）。`D_avg` 之 `round(...,2)` 係其**進入乘積**之法定記載鏈
        （辦法 §3），與本欄之**門檻比較**用途不同，**勿互代**。
    ...
    **缺 FRONT_LINE／BASELINE ⇒ loud `RuntimeError`**（施工單 §一：禁 fallback 回方法 B）。
    ...
    return {'D_avg': round(D_avg, 2), 'D_min': round(D_min, 2), 'D_max': round(D_max, 2),
            ...
            # 🆕 K-9-2 剖面欄（全精度·未 round·勿與 D_min 互代——見上 docstring）
...
def k92_block_depth_check(build_blocks, depth_info_by_label, front_road_width_by_label):
    """**K-9-2 街廓層剖面深度警示**（KL 裁 2026-08-01·canonical：
    `grep -n "^## K-9 " docs/rulings/K-6_街角地分配程序與可分配判準.md`）。

    **判準（正典逐字）**：前緣線上每一點沿 **BASELINE 法向**至 BASELINE 之垂距，
    **任一點 ≤ 畸零地最小深度即警示**（**等於也警示**，因係逐點比較而非平均）。

    **解析**：前緣線與 BASELINE 皆直線 ⇒ 垂距沿弦**線性** ⇒ 最小值必落於前緣線**兩端之一**
    ⇒ 逕取 `_compute_block_depth_alloc` 之 `D_k92_min`。⛔ **禁取樣、禁掃描、禁迭代兜底。**
    （與 `D_avg` 之「平均＝兩端平均」同一線性性質——**同一組數字、一個取平均一個取最小**。）

    🔴 **兩個「最小深度」同名不同量·嚴禁互代**（K-9-6-a）：
      · **量測值** ＝ 本函式之 `D_k92_min`（該街廓剖面之最小深度·本案 33〜47m）
      · **門檻值** ＝ 畸零地**附表** `get_min_lot_size(分區, 正面路寬)['min_depth']`（本案 14.00）
      ⇒ 此處係附表門檻之**正確**用途（K-9-6：附表值＝**可建築門檻**）；
        與 **GB-13** 所述「附表值被誤用為**量測帶深**」係**兩回事**、勿混。
    ...
    **警示、非停機**（正典用「警示」二字）：本函式只回傳 `warn` 旗標，**呈現層自行決定**；
    ...
    build_blocks               可建築街廓 dict 清單（需 `label`／`category`）
    depth_info_by_label        `n19p_depth_info_by_label(...)` 之回傳（**唯一深度算法源**）
    front_road_width_by_label  `{label: 正面路寬(m)}`
                               （app＝`sb['rows']`：`grep -n "sb_rows_by_label = {" app.py`；
                                harness＝`run_verification.build_param_table` 之 `正面路寬(m)`）
    回傳 逐街廓 dict 之 list，欄位：
      `label`／`category`／`front_road_width_m`／`d_front_p1`／`d_front_p2`／
      `D_k92_min`（量測值·全精度）／`min_depth_table`（附表門檻）／`margin`（餘裕＝量測−門檻）／
      `warn`（`D_k92_min <= min_depth_table` ⇒ True）

    🔒 **比較用未 round 之原值**、`round()` 只在輸出層（K-8 §二-1 WARNING-B 體例）。

    **缺件一律 loud `RuntimeError`**（施工單 §3.4：禁靜默、**不得跳過該街廓**）：
      缺深度資訊／缺 `D_k92_min`／缺正面路寬（≤0）／附表查無深度（≤0）。
    ⚠️ 缺 **FRONT_LINE／BASELINE** 於**上游** `_compute_block_depth_alloc` 即已 loud raise，
      本函式不重複攔截、**不留 except**。
    """
    _dinfo = depth_info_by_label or {}
    _fwb = front_road_width_by_label or {}
    _rows = []
    for b in (build_blocks or []):
        _lbl = b['label']
        _cat = b.get('category', '') or ''
        _di = _dinfo.get(_lbl)
        if not _di:
            raise RuntimeError(
                f"🔴 K-9-2 街廓層深度檢查：街廓 {_lbl} 無 N-19′ 深度資訊"
                f"（`n19p_depth_info_by_label` 之母體不含此街廓）。"
                f"⛔ 禁跳過該街廓——K-9-2 係全街廓逐格檢查。")
        _pmin = _di.get('D_k92_min')
        if _pmin is None:
            raise RuntimeError(
                f"🔴 K-9-2 街廓層深度檢查：街廓 {_lbl} 之深度資訊缺 `D_k92_min`"
                f"（＝前緣線兩端垂距之 min）。來源應為 `_compute_block_depth_alloc`；"
                f"⛔ 禁改讀 `D_min`（方法 A 取樣診斷·同名不同量·K-9-6-a）。")
        _fw = float(_fwb.get(_lbl, 0.0) or 0.0)
        if _fw <= 0:
            raise RuntimeError(
                f"🔴 K-9-2 街廓層深度檢查：街廓 {_lbl} 之**正面路寬**缺值（{_fw}）。"
                f"畸零地附表係以「分區 × 正面路寬」查得最小深度，缺路寬 ⇒ 門檻不可定；"
                f"⛔ 禁以附表最窄一列（路寬 ≤7m）兜底充數（no-silent-fallback）。"
                f"請先於臨街負擔步驟填入該街廓正面路寬。")
        _md = float(get_min_lot_size(_cat, _fw).get('min_depth', 0.0) or 0.0)
        if _md <= 0:
            raise RuntimeError(
                f"🔴 K-9-2 街廓層深度檢查：街廓 {_lbl}（分區「{_cat}」·正面路寬 {_fw:g}m）"
                f"於畸零地附表查無最小深度（得 {_md}）。"
                f"⛔ 禁以任何常數兜底——門檻無法源即不得比較。"
                f"請檢查該街廓分區是否為可建築土地、或補 `HUALIEN_MIN_LOT_TABLE` 之對應列。")
        _pmin = float(_pmin)
        _rows.append({
            'label': _lbl, 'category': _cat, 'front_road_width_m': _fw,
            'd_front_p1': float(_di['d_front_p1']), 'd_front_p2': float(_di['d_front_p2']),
            'D_k92_min': _pmin, 'min_depth_table': _md,
            'margin': _pmin - _md,
            # 🔒 「等於也警示」⇒ 用 `<=`，**禁**寫成 `<`
            'warn': (_pmin <= _md),
        })
...
def parcel_min_width_n14(cut_coords, d_hat, front_pt, min_depth, _label=''):
    ...
    **二側境界線間平行 FRONTLINE 之距離**之**最小值**。
    ...
    ── 演算法（**精確·非取樣·非 GEOS**）──────────────────────────────────────
    令 `t` ＝ 沿 FRONT 法向（指向宗地內）之深度、`s` ＝ 沿 FRONT 之橫向座標。
    宗地為多邊形 ⇒ 其於深度 `t` 之**平行 FRONTLINE 弦長** `w(t)` 為 **分段線性**函式
    ⇒ 極小值必落於**斷點**：`{t_lo, min_depth} ∪ {各頂點之 t}∩[t_lo, min_depth]`。
    ...
    # ── 🔒 E-10：改於 **(s,t) 局部框**作業（KL 域裁之法定粒度 0.01 為據·reviewer 活抓）──
    #   `s = (P−front_pt)·d̂`／`t = (P−front_pt)·n̂` ＝ **剛體變換**（旋轉＋平移）
    #   ⇒ 面積／有效性／凸性**逐一不變**，而座標量級自 TWD97 ~3.1e5 降至 O(1e2)
    #   ⇒ 浮點絕對噪訊自 ~1e-11 降至 ~1e-14。
    _P = _np_w.asarray([[float(p[0]), float(p[1])] for p in cut_coords], dtype=float) - _fp
    _sv_all = _P @ _d
    _tv_all = _P @ _n
    _poly = _Poly_w(list(zip(_sv_all.tolist(), _tv_all.tolist())))
    ...
    # 法向取號：指向宗地內（形心側）。(s,t) 框下即「形心之 t ≥ 0」。
    ...
    #   KL 交辦文原寫 `min(_tv) > 1e-6`；倉內正典早已明文「直弦離邊界 ~2mm →
    #   **線上判定容差 ≥1cm**（`perp_tol=0.05` 實用），**`1e-6` 會漏量**」
    #   ——`1e-6` 正是該處點名之失效值（實測 0m 25 筆／3.5m 23 筆會被誤咬）。
    #   理由與 C-5 同族：**低於法定記載粒度之偏移，在法規上不可能被記載為「不臨街」**。
    #   ⛔ **依據不是「實測留 N 倍餘量」**（N-17 明禁以觀測殘差定閘寬）——實測值僅供
    #      「本閘不誤咬真值」之**安全性佐證**，非其**依據**。
    #
    #   🔒 **雙向**（claude.ai 判定之施工完整性要求·**純技術·非 KL 裁示**）：
    #      負向＝宗地越過正面路街線進入道路，同屬上游錯誤 ⇒ 判準取 `abs(min(_tv))`。
    #
    #   ── 實測母體與上下界（**母體聲明**·失敗考古 #30 之直接對治）─────────────────
    #      舊註解寫「實測最大 2.47e-4、低容差 40 倍」，其統計母體**只涵蓋會觸發的正側子集**
    #      ⇒ 以子集上界冒充全體上界。正確之全體（59 宗業主宗·`min(_tv)` 本身）：
    #        · **正側**上界 **+2.4746e-04**（R1 `628(5)`·街角宗）
    #          ／非街角宗之正側上界 **+2.3953e-04**（R1 `628-34(3)`）
    #        · **負側**下界 **−1.4826e-02**（R4 `628-1(1)`·街角宗）——**已超本容差**，
    #          正是 E-7 於量測前跳過之族；其成因為截角，非「未臨街」。
    #      ⇒ 對**現行**路徑（街角宗依 E-7 `continue`）本閘之觸發數為 **0**。
    #
    #   ── 前提句「重劃後每宗均應臨正街」之泛用性查證（W-G.5·2026-07-27·**已結案**）──
    #      疑點：倉內另有「**未臨正街**」語彙（`grep -n "未臨正街" app.py`），
    #            若業主宗可能整宗未臨正街，本閘之前提句對其他案件即不成立。
    #      查證（依碼·非依實測）：
    #        · 「未臨正街」係**半平面判別語彙**（left `block∩{s<0}`／right `block∩{s>s(p2)}`），
    #          **非**輸出類別——`g_rows` 之 `推進側別` 僅 `{left, right, 抵費地}`。
    #          其**面積實體**＝`frag_poly`（碎片·補丁十「乙」：半平面僅判別、不供面積）。
    #        · `R_end = 未臨正街 ∪ 末端帶`（`_end_region_R`），**末端帶**過 `end_pt`
    #          （＝FRONT 端點）之 `min_width` ⊥ 帶 ⇒ `R_end` **恆含臨正街構件**。
    #        · 業主宗占 `R_end` 之門檻＝`G ≥ area(R_end)`（`grep -n "_end_gate" verify/wf_f4.py`），
    #          且其幾何 ＝ `_fuse(strip_at(0, S_new), wedge)`——`strip_at` 係
    #          `block ∩ 一段 s 區間`、自街廓角落**連續單調**鋪出
    #          ⇒ 面積既 ≥ area(R_end) ⊋ area(wedge)，該 s 區間**必越過** `s(p2)`
    #            進入臨正街段，且其臨街段長 **≳ min_width**（因 `R_end ⊇ 末端帶`）。
    ...
    if abs(min(_tv)) > _EPS_TOUCH_FRONT:
        raise RuntimeError(
            ...
            f"（距 FRONT 最近之頂點 {min(_tv):+.6f}m·|偏移| > 容差 {_EPS_TOUCH_FRONT}m）。"
    ...
    if max(_tv) < _md:
        raise RuntimeError(
            ...
            f"（手冊 二(一)3）⇒ **分配線不合規**，停")
    # E-8a 凸性（KL 裁 2026-07-27·claude.ai 立）
    #   可達宗地形狀集合①②③**全為凸多邊形** ⇒ 非凸即上游有誤。
    #   閘寬 `0.01㎡` ＝ **面積之法定粒度**（實施辦法 §3 第三位四捨五入·與 C-5／E-1′ 同族），
    #   **非**實測殘差。實測安全性（**佐證·非依據**）：59 宗之 `hull.area − area`
    #   上界 **1.3474e-09㎡**（R6 `628-18(1)`），離閘寬 7 個數量級。
    #   ⇒ 一擊同滅：尖刺（可越過 E-2′ 之深度判準）／U 形／多段弦。
    #   🔒 **適用域＝原位次宗形（①②③）**。E 世代之 `_fuse(cut, wedge)`（`wf_f4` E3）
    #      **未必凸** ⇒ **本閘不得外推至 E 世代幾何**（元模式：正典條文只在其落點適用）。
    _EPS_CONVEX = 0.01
    _conv_gap = float(_poly.convex_hull.area - _poly.area)
    if _conv_gap > _EPS_CONVEX:
        raise RuntimeError(
            f"🔴 parcel_min_width_n14[{_label}]：宗地非凸"
            f"（convex_hull − poly ＝ {_conv_gap:.4f}㎡ > 法定粒度 {_EPS_CONVEX}㎡）。"
            f"**可達形狀集合①②③全為凸多邊形** ⇒ **上游幾何有誤**，停"
            f"（⛔ 禁回數字——尖刺／U 形會被當成寬度）")
    _t_hi = _md
    # ── 🔒 E-9 量測起點 ＝ **前緣邊之上緣**（W-G.5·本波立）──────────────────────────
    #   **舊式 `_t_lo = max(0, min(_tv))` 是錯的**：取「全宗最低頂點」之深度。
    #   `cut_coords` 之前緣邊與 FRONT_LINE **近平行但非嚴格平行**（實測兩端 t 差
    #   5e-8～5.5e-5 m）⇒ 於 `min(t)` 之弦**只切到前緣邊的一個角**
    #   ⇒ 回一個 **0.03m 級的薄片長度**並被下游當作宗地寬度
    #   （實測 R1 `628-35(2)`：舊式回 **0.0327**，其真實臨街邊長 **18.1027**）。
    #
    #   正解：凡 `|t| ≤ 法定粒度` 之頂點，**法律上即位於正面路街線上**
    #   ⇒ 前緣諸頂點同屬 `t=0` 之等價類 ⇒ 量測帶起點取該等價類之**上緣**，
    #     使該處之弦**橫跨整條前緣**。
    #
    #   🔒 **邊基·非頂點門檻基**（reviewer WARNING）：以「**兩端點皆在容差內之邊**」
    ...
    # ── 🔒 E-10 弦長**解析求**（禁 GEOS·見 docstring）────────────────────────────
    #   凸多邊形於 `t=T` 之弦 ＝ `max(s交) − min(s交)`。交點由邊之線性內插得，
    #   **恰過頂點者兩鄰邊各回同一 s**（重根）⇒ 單點接觸自然得弦長 0（見 E-8b）。
    def _chord_len_at(_T):
        _xs = []
        for _i in range(len(_ring) - 1):
            _s0, _t0 = _ring[_i]
            _s1, _t1 = _ring[_i + 1]
            if _t0 == _t1:
                if _t0 == _T:                     # 邊與量測線共線 ⇒ 兩端點皆為交點
                    _xs.append(_s0); _xs.append(_s1)
                continue
            if (_t0 - _T) * (_t1 - _T) <= 0.0:
                _u = (_T - _t0) / (_t1 - _t0)
                _xs.append(_s0 + _u * (_s1 - _s0))
        return (None if not _xs else (max(_xs) - min(_xs), len(_xs)))

    # E-8b 單點接觸（弦長 0）⇒ loud raise。
    #   `_EPS_ZERO_CHORD` ＝ **數值零**判定，**非**法律門檻：(s,t) 局部框座標量級 O(1e2)
    #   ⇒ 雙精度絕對噪訊 ~1e-14；取 1e-9 為零判定。
    #   ⚠️ **不得改用法定粒度 0.01m**——本案存在**真實 5.87mm 臨街邊**
    #      （R6 `628-53(1)`；R2 `628-43(1)` 為 5.98mm），0.01 門檻會把真值咬成錯誤。
    _EPS_ZERO_CHORD = 1e-9
    _wmin = None
    for _t in sorted(_ts):
        _res = _chord_len_at(_t)
        # E-3′ 帶內每斷點之弦須存在
        #   ⛔ **已撤掉舊之 `_ln.length` 總長語意**：多段加總會**製造一個不存在的寬度**。
        #      非單段之情形已由 **E-8a 凸性**於量測前根除（凸多邊形之弦恆為單段）。
        #   附註：「U 字形宗地」之讀法選項經 KL 裁定作廢 ⇒ **禁實作任何讀法選項**。
        if _res is None:
            raise RuntimeError(
                f"🔴 parcel_min_width_n14[{_label}]：深度 t={_t:.6f}m 處之弦"
                f"與宗地無交點——**帶內每斷點皆應有弦** ⇒ 宗地幾何違反分配模型，停")
        _w, _nx = _res
        if _w <= _EPS_ZERO_CHORD:
            raise RuntimeError(
                f"🔴 parcel_min_width_n14[{_label}]：深度 t={_t:.6f}m 處之弦長為 "
                f"{_w:.3e}m（≤ 數值零 {_EPS_ZERO_CHORD}·交點 {_nx} 個）＝**單點接觸**。"
                f"**法律上單點不構成臨街** ⇒ 上游幾何有誤，停")
...
# ══ 🆕 E-7 顯性未決態（KL 裁 2026-07-27 ＋ claude.ai BLOCKED-1 2026-07-27）══════════════
#   **禁以空白／None 代表未決**——未決必須是**具名的第三態**，否則下游之
...
# 🔴 **K-9-5 已撤銷本豁免**（K-7 作廢·KL 裁 2026-08-01·正典
#   `grep -n "^## K-9 " docs/rulings/K-6_街角地分配程序與可分配判準.md`）───────────────
#   K-9-5：街角第 1 宗**寬度要實量**（**截角前**幾何），門檻 ＝ **退縮寬 ＋ 畸零地最小寬**；
#   K-9-3：深度改**街廓層**檢查（K-9-2）⇒ **宗地層一律不量深度**。
#   ⇒ 上段所述之「面積門檻蘊含寬度與深度」**已非現行判準**。
#   ⚠️ 現行碼**尚未實作實量**（實作屬 **K-6-A2**）⇒ **上段描述之現況為過渡態**。
#   ⛔ 下行常數之**名稱與字串值於本批一律不動**：`_K4` 正名與措辭更新屬**波末重烤批**
#      （遷移項登記見上開正典之「K-8 波末遷移項登記」節）。
...
def evaluate_parcel_width_n14(row, front_line, min_depth, legal_min_width,
    ...
    留在 `main()` 內即 **headless 不可測** ⇒ claude.ai 要求之機器閘
    「pending 筆數 == 街角 winner 筆數」**無從落地**（只能寫成 AST／prose gate＝N0-17-b 之忌）。
    抽出後 `harvest()` 取得到 ⇒ 探針可對**實資料**逐宗跑真判定。
    （同 §7 接線鐵律之 `_build_wf_ctx`：module 級純函式·+1 gate 可測。）

    ── 回傳 ────────────────────────────────────────────────────────────────────
    `dict` 之欄位增量（呼叫端 `row.update(...)`）：
    ...
    ── 🔴 **K-9-5 已撤銷本豁免**（K-7 作廢·KL 裁 2026-08-01）────────────────────────
    正典：`grep -n "^## K-9 " docs/rulings/K-6_街角地分配程序與可分配判準.md`
    · **K-9-5**：街角第 1 宗**寬度要實量**（**截角前**幾何·§4 末段），
      門檻 ＝ **退縮寬 ＋ 畸零地最小寬**；其餘各宗門檻 ＝ **畸零地最小寬**。
    · **K-9-3**：深度改**街廓層**檢查（K-9-2）⇒ **宗地層一律不量深度**（含街角第 1 宗）。
    · **K-9-1**：街角第 1 宗**不豁免**街廓最小建築面積。
    ⇒ 上段「蘊含寬度、深度皆合格」**已非現行判準**。
    ⚠️ 本函式**尚未實作實量**（實作屬 **K-6-A2**）⇒ **上段描述之現況為過渡態**，
       其回值與字串值於本批**一律不動**。
    📐 八街角之實測寬度底料（K-6-A2 上閘前之 KL 過目材料）：
       `verify/probes/probe_ruling_K9_corner_width.py`（**只量不判**）。
    ...
    if _side in ('抵費地', '🟠 孤立公設地', '💰 現金補償'):
        return {'寬度判定': WIDTH_VERDICT_NA}
    if not (front_line and front_line.get('p1') and front_line.get('p2')):
        raise RuntimeError(
    ...
    if _lw > 0 and _w < _lw:
        ...
        #   G 係地主依重劃前面積與地價計得之**應分配面積**；壓低帳面以逼合併
        #   ＝ **無償轉入調配池**。實配短少應走**差額地價補償**。
        _out['寬度判定'] = WIDTH_VERDICT_BAD
        _out['_below_min_width'] = True
        _out['_width_violation_note'] = f"寬度 {_w:.2f}m < 法定最小 {_lw:.2f}m"
    else:
        _out['寬度判定'] = WIDTH_VERDICT_OK
    return _out


def width_verdict_of(row):
    """自 row 之旗標**還原**寬度判定（四態·**恆有值·禁回 None**）。
    ...
    **不應再出現**（`_width_chamfer_pending` 分支保留**僅供偵測殘留**，由機器閘咬）。
    ...
    🔴 **K-9-5 已撤銷該豁免**（K-7 作廢·KL 裁 2026-08-01·正典
    `grep -n "^## K-9 " docs/rulings/K-6_街角地分配程序與可分配判準.md`）：
    街角第 1 宗改**寬度實量**（截角前），門檻＝退縮寬＋畸零地最小寬。
    **本函式尚未改**（實作屬 **K-6-A2**）⇒ 上段所述之四態現況為**過渡態**。
    """
    if (row or {}).get('寬度判定'):
        return str(row['寬度判定'])
    ...
    if bool((row or {}).get('_below_min_width', False)):
        return WIDTH_VERDICT_BAD
    if (row or {}).get('實際寬度(m)') is not None:
        return WIDTH_VERDICT_OK
    return WIDTH_VERDICT_NA
...
def merge_subparcels_by_parent(g_rows: list, min_area_by_block: dict) -> dict:
    ...
    for parent, rows in by_parent.items():
        ...
        for r in rows:
            ...
            # ── 🆕 E-7 三態（claude.ai BLOCKED-1 2026-07-27）────────────────────────
            #   ⛔ 舊碼 `_below_width = bool(r.get('_below_min_width', False))`：
            #      街角宗（`待判`）之該旗標**不存在** ⇒ 讀成 `False` ⇒ 以「**寬度合格**」
            ...
            # ── 🔴 **K-9-5 已撤銷本豁免**（K-7 作廢·KL 裁 2026-08-01·正典
            #   `grep -n "^## K-9 " docs/rulings/K-6_街角地分配程序與可分配判準.md`）──
            #   街角第 1 宗改**寬度實量**（截角前），門檻＝退縮寬＋畸零地最小寬（K-9-5）；
            #   深度改街廓層檢查、宗地層不量（K-9-2／K-9-3）。
            #   ⚠️ **本處尚未改**（實作屬 **K-6-A2**）⇒ 上段所述為**過渡態**。
...
def select_corner_lots_both_sides_v12(
    ...
    #   🆕 P-C：require_g_map ⇒ 真 G **側特定**——p1 組吃 `_G_true_p1`、p2 組吃 `_G_true_p2`
    #   （呼叫端 run_corner_pk/app 於分流前逐候選逐側算好·存 cand）。缺值不入 map ⇒ require raise。
    if require_g_map:
        _map_p1 = {c['暫編地號']: c['_G_true_p1'] for c in p1_group
                   if c.get('_G_true_p1') is not None}
        _map_p2 = {c['暫編地號']: c['_G_true_p2'] for c in p2_group
                   if c.get('_G_true_p2') is not None}
    else:
        _map_p1 = _map_p2 = g_values_map
    return {
        'p1_end': _pk_one_side_v12(p1_group, _map_p1, base_front_len_m,
                                   require_g_map=require_g_map),
        'p2_end': _pk_one_side_v12(p2_group, _map_p2, base_front_len_m,
                                   require_g_map=require_g_map),
    }
...
def _pk_one_side_v12(group: list, g_values_map: dict,
    ...
    for cand in group:
        ...
        # 🆕 P-C（裁定M·Q1）：達標決策用 cand_G（require_g_map=True ⇒ **側特定真 G**）；
        #   `G_for_threshold`（→診斷『G估(㎡)』欄）**語意不變＝estG**·與達標 G **解耦**
        #   （真 G 為中性口徑·T1 證零翻盤·達標/選中集不動 ⇒ G估欄不動 ⇒ v3·診斷 綠·名目不動）。
        #   非-require 路徑：cand_G 即 estG（g_values_map[pid]==cand['G_estimated']）⇒ 本行零行為。
        cand['_G_true'] = round(cand_G, 2) if require_g_map else None   # 真G(㎡) 欄來源（側特定）
        _est_for_col = cand.get('G_estimated', cand.get('G_value', cand_G))
        cand['G_for_threshold'] = round(float(_est_for_col or 0), 2)
...
def _corner_block_true_G(*, candidates, a_by_pid, zone_by_pid, blk_poly, corner_pt,
    ...
    禁另寫平行式#20）。側無 SIDE_LINE（`has_*=False` 或 `side_mid None`）⇒ 該側 None（不 solve·不 raise）。
    app（Step L·session B/C）與 harness（run_corner_pk·`_compute_v3_finance`）**共用本函式**——
    幾何/財務由呼叫端各路徑組裝（plan v4 §3.1 分寫）·solve 單一真相源在此。"""
    out = {}
    for c in candidates:
        pid = c.get('暫編地號', '')
        a = a_by_pid.get(pid)
        ...
        if a and float(a) > 0:
            ...
            A_ratio = ((post_price_blk / pre_p)
                       if (pre_p > 0 and post_price_blk > 0) else 1.0)
            if has_left and side_mid_left is not None:
                gp['p1'] = round(_corner_first_lot_G(
                    a_m2=float(a), A_ratio=A_ratio, B=B, C=C, l_front=l_front,
                    l_side=l_side_left, F=F_left, block_poly=blk_poly, d_hat=d_hat,
            ...
            if has_right and side_mid_right is not None:
                gp['p2'] = round(_corner_first_lot_G(
                    a_m2=float(a), A_ratio=A_ratio, B=B, C=C, l_front=l_front,
                    l_side=l_side_right, F=F_right, block_poly=blk_poly, d_hat=d_hat,
                    corner_pt=corner_pt, s_max_left=s_max_left, s_max_right=s_max_right,
                        ...
                        for _row in (_corner_rows_init or []):
                            ...
                            if _use_v13:
                                ...
                                try:
                                    ...
                                    _slb_ap = (st.session_state.get('f3_cad_side_lines_by_side', {})
                                               or {}).get(_lbl, {}) or {}
                                    _smL_ap = (_slb_ap.get('left') or {}).get('mid')
                                    _smR_ap = (_slb_ap.get('right') or {}).get('mid')
                                    _alloc_ap = (st.session_state.get('f3_cad_alloc_dir', {})
                                                 or {}).get(_lbl)
                                    _verts_ap = (_blk_meta_for_side or {}).get('vertices')
                                    _p1_ap = np.array(_fl_p1_lstep, float)
                                    _p2_ap = np.array(_fl_p2_lstep, float)
                                    _sL_ap = float(np.linalg.norm(_p2_ap - _p1_ap))
                                    _dh_ap = (_p2_ap - _p1_ap) / (_sL_ap or 1.0)
                                    _bpoly_ap = _SP_pc(_verts_ap)
                                    if not _bpoly_ap.is_valid:
                                        _bpoly_ap = _bpoly_ap.buffer(0)
                                    _ax_ap = alloc_normal_axis(_alloc_ap) if _alloc_ap else None
                                    _sR_ap = (_oblique_s_max(_verts_ap, _dh_ap, _p1_ap, _ax_ap)
                                              or _sL_ap) if _ax_ap is not None else _sL_ap
                                    _depth_ap = float((st.session_state.get(
                                        'f3_alloc_depth_by_label', {}) or {}).get(_lbl, 0) or 0)
                                    _tab6_ap = float(st.session_state.get(
                                        'f3_total_burden_rate_from_finance', 0) or 0)
                                    _a_ap = {}; _zone_ap = {}
                                    for _c_ap in _candidates:
                                        _pid_ap = _c_ap['暫編地號']
                                        _tp_ap = next((tp for tp in (temp_parcels or [])
                                                       if tp.get('暫編地號') == _pid_ap), None)
                                        if _tp_ap is not None:
                                            _a_ap[_pid_ap] = (
                                                round(float(_tp_ap.get('分攤登記面積_m2', 0) or 0)
                                                      + float(_tp_ap.get('面積_m2', 0) or 0), 2)
                                                if '分攤登記面積_m2' in _tp_ap
                                                else round(float(_tp_ap.get('面積_m2', 0) or 0), 2))
                                            _zone_ap[_pid_ap] = _tp_ap.get('重劃前地價區段', '')
                                    _true_map_ap = _corner_block_true_G(
                                        candidates=_candidates, a_by_pid=_a_ap, zone_by_pid=_zone_ap,
                                        blk_poly=_bpoly_ap, corner_pt=_p1_ap, d_hat=_dh_ap,
                                        s_max_left=_sL_ap, s_max_right=_sR_ap, alloc_dir=_ax_ap,
                                        side_mid_left=_smL_ap, side_mid_right=_smR_ap,
                                        l_front=float(_sbr_ap.get('正街尺度', 0) or 0),
                                        l_side_left=float(_sbr_ap.get('左側尺度', 0) or 0),
                                        l_side_right=float(_sbr_ap.get('右側尺度', 0) or 0),
                                        F_left=float(_sbr_ap.get('左側路寬(m)', 0) or 0),
                                        F_right=float(_sbr_ap.get('右側路寬(m)', 0) or 0),
                                        B=float(B_value or 0), C=float(C_for_calc or 0),
                                    ...
                                    for _dg_pass, _dg_list in (('達標', _dg_res.get('qualified', [])),
                                        ...
                                        for _dc in (_dg_list or []):
                                            _corner_cand_diag.append({
                                                ...
                                                # 🆕 P-C（裁定M·Q1）：達標決策所用之側特定真 G（G估欄保 estG）
                                ...
                                for _entry, _res in (left_results + right_results):
                                    ...
                                    if len(_coords) >= 3:
                                        try:
                                            ...
                                            _p = None
                                    # 停機③ 修（KL 2026-07-20·約束1「判準對齊 ΣG 成員」）：凡算進 ΣG 之
                                    #   owner 宗皆須自池扣其幾何——**廢 area≥0.5 sliver 濾**（1b290e4 遷移遺留·
                                    #   無職·退化守衛係 len≥3/is_valid/is_empty·非此常數·#26a 已核）。舊 area≥0.5
                                    #   剔除合法小宗（如 R2 628-43(1) 0.27㎡）致池雙計 → 停機③ |Σ(G−幾何)|=0.28
                                    #   （=0.0124 owner 殘差＋0.2681 雙計宗·逐位）。
