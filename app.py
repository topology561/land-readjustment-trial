...
def _inflate_a_for_orphan(orphan: dict, target_buildable_parcels: list,
                            ownership_map: dict) -> dict:
    """
    🆕 Phase 9.8：將公設地之 a 灌入該 owner 在目標街廓內之 buildable parcel（trial 用）

    分配策略：
      - 同 parent 地號 → 全部灌入該 parent 之 buildable
      - 同歸戶但不同 parent → 平均分配給所有同歸戶 buildable

    回傳：modified parcels list（深拷貝，不影響原資料）
    """
    if not orphan or not target_buildable_parcels:
        return list(target_buildable_parcels or [])
    try:
        orph_parent_norm = _normalize_landno_module(orphan.get('原地號', ''))
        orph_gid = _resolve_ownership(orphan.get('原地號', ''), ownership_map)
    except Exception:
        return list(target_buildable_parcels or [])
    orph_area = float(orphan.get('幾何面積_m2', orphan.get('面積_m2', 0)) or 0)
    if orph_area <= 0:
        return [dict(tp) for tp in target_buildable_parcels]

    # Tier 1：同 parent 地號 buildable
    same_parent = []
    same_owner = []
    for tp in target_buildable_parcels:
        try:
            tp_parent_norm = _normalize_landno_module(tp.get('原地號', ''))
            tp_gid = _resolve_ownership(tp.get('原地號', ''), ownership_map)
        except Exception:
            tp_parent_norm = ''
            tp_gid = ''
        if tp_parent_norm and tp_parent_norm == orph_parent_norm:
            same_parent.append(tp)
        elif orph_gid and tp_gid == orph_gid:
            same_owner.append(tp)

    # 深拷貝避免污染原資料
    modified = [dict(tp) for tp in target_buildable_parcels]
    targets = []
    if same_parent:
        # 灌入同 parent 之 parcels
        targets = [m for m in modified
                   if _normalize_landno_module(m.get('原地號', '')) == orph_parent_norm]
    elif same_owner and orph_gid:
        targets = [m for m in modified
                   if _resolve_ownership(m.get('原地號', ''), ownership_map) == orph_gid]
    if not targets:
        return modified
    share = orph_area / len(targets)
    for t in targets:
        t['面積_m2'] = float(t.get('面積_m2', 0) or 0) + share
        t.setdefault('_inflated_from_orphans', []).append({
            'orphan_暫編': orphan.get('暫編地號', ''),
            'orphan_原地號': orphan.get('原地號', ''),
            'inflated_area': round(share, 2),
        })
    return modified


# ═══════════════════════════════════════════════════════════════════════
# 🚨 Patch D-3：三層調配機制（取代 _premerge_public_land_to_a + _consolidate_offset_polygons）
# Tier 1: 同 Ri + 同歸戶 → a' 加進 buildable.a → 後續 G 重算（純帳面）
# Tier 2: 跨 Ri 純孤立 → 從目標 Ri 之中間抵費地 polygon 物理切走 a' 子區塊（純幾何）
# Tier 3: 抵費地殘餘 < min_area 或寬度 < min_width → 平均增配 G（含 _treat_as_offset 納入）
# ═══════════════════════════════════════════════════════════════════════

def _polylabel_inscribed_diameter(poly) -> float:
    """🚨 Patch D-3 helper：取 polygon 內接圓直徑（近似寬度）"""
    try:
        from shapely.ops import polylabel as _pl_d3
        center = _pl_d3(poly, tolerance=0.5)
        return float(poly.boundary.distance(center)) * 2.0
    except Exception:
        try:
            mbr = poly.minimum_rotated_rectangle
            cs = list(mbr.exterior.coords)
            edges = [((cs[i + 1][0] - cs[i][0]) ** 2
                      + (cs[i + 1][1] - cs[i][1]) ** 2) ** 0.5
...
def _geometric_carve_from_side(poly, target_area: float, near_point=None):
    """
    🚨 Patch D-3 helper：從 polygon 切出 target_area 大小子區塊（沿主軸 sweep + 二分法）

    回傳：(切片 polygon, 殘餘 polygon)
    """
    from shapely.geometry import Polygon as _SP_d3
    if poly is None or poly.is_empty or target_area <= 0:
        return None, poly
    if poly.area <= target_area + 0.5:
        return poly, _SP_d3()
    try:
        mbr = poly.minimum_rotated_rectangle
        cs = list(mbr.exterior.coords)
        ...
        return None, poly
    try:
        proj_pts = [(x * ux + y * uy) for x, y in poly.exterior.coords]
        proj_min = min(proj_pts); proj_max = max(proj_pts)
    except Exception:
        return None, poly
    ...
    if near_point is not None:
        try:
            np_proj = float(near_point[0]) * ux + float(near_point[1]) * uy
            from_min_end = (abs(np_proj - proj_min) < abs(np_proj - proj_max))
        except Exception:
            from_min_end = True
    else:
        from_min_end = True
    bnd = poly.bounds
    half_size = max(bnd[2] - bnd[0], bnd[3] - bnd[1]) * 4.0 + 100.0
    nx, ny = -uy, ux

    def _make_half_plane(cut_proj_val, take_below):
        if take_below:
            lo_p, hi_p = proj_min - 1.0, cut_proj_val
        else:
            lo_p, hi_p = cut_proj_val, proj_max + 1.0
        c1 = (lo_p * ux - half_size * nx, lo_p * uy - half_size * ny)
        c2 = (hi_p * ux - half_size * nx, hi_p * uy - half_size * ny)
        c3 = (hi_p * ux + half_size * nx, hi_p * uy + half_size * ny)
        c4 = (lo_p * ux + half_size * nx, lo_p * uy + half_size * ny)
        return _SP_d3([c1, c2, c3, c4, c1])

    lo, hi = proj_min, proj_max
    cut_geom = None
    for _ in range(40):
        mid = (lo + hi) / 2.0
        try:
            half = _make_half_plane(mid, take_below=from_min_end)
            cut_geom = poly.intersection(half)
            cut_area = float(cut_geom.area) if hasattr(cut_geom, 'area') else 0.0
        except Exception:
            return None, poly
        if abs(cut_area - target_area) < 0.5:
            break
    ...
    if cut_geom is None or cut_geom.is_empty:
        return None, poly
    try:
        residual = poly.difference(cut_geom)
        if residual.is_empty:
            residual = _SP_d3()
        if hasattr(cut_geom, 'geoms') and cut_geom.geom_type == 'MultiPolygon':
            cut_geom = max(cut_geom.geoms, key=lambda g: g.area)
        if hasattr(residual, 'geoms') and residual.geom_type == 'MultiPolygon':
            residual = max(residual.geoms, key=lambda g: g.area)
        return cut_geom, residual
    except Exception:
        return cut_geom, _SP_d3()


# 🚦 W-C 裁示：同歸戶「公設地→建地」補償（Tier 1 a_prime）延至 W-F。
#   W-C 階段不注入 a_prime（建地 面積_m2 維持 0 → a = 分攤登記面積_m2）；
#   仍執行同歸戶配對並把該公設地標記為已認列（不落入 Tier 2、維持為公設地、W-C 不配）。
#   Tier 1 邏輯整段保留，W-F 啟用補償時把本旗標改為 True 即可。
_TIER1_APRIME_INJECT_ENABLED = False  # W-F 再啟用


def _allocate_three_tier_v1(temp_parcels: list,
                              classified_blocks: list,
                              ownership_map: dict,
                              pre_price_by_zone: dict,
                              min_area_by_block: dict,
                              legal_min_width: float = 3.5,
                              burden_factor: float = 0.55) -> dict:
    """
    🚨 Patch D-3：三層調配主邏輯（取代 _premerge_public_land_to_a）
    Tier 1：同 Ri 同歸戶 → 修改 buildable.a 值（後續 Step G 重算 G 自然反映）
    Tier 2/3 由 _allocate_tier2_tier3_geometric 後續處理。
    """
    from shapely.geometry import Polygon as _P_d3a
    BUILDABLE_CATS = {'住宅區', '商業區'}
    PUBLIC_CATS = {'道路', '溝渠', '廣場', '鄰里公園', '兒童遊樂場',
                    '綠地', '零售市場'}
    tier1_log = []
    cash = []
    consumed_pub_ids = set()
    if not temp_parcels:
        return {'tier1_log': [], 'updated_parcels': [],
                '_tier2_pending_pubs': [], 'cash_compensation': []}
    # 🚨 Patch E-0.4：排除 ghost sliver（_is_ghost_sliver=True 不參與分配計算）
    _ghost_filtered_t1 = sum(1 for tp in temp_parcels
                              if tp.get('_is_ghost_sliver', False))
    temp_parcels = [tp for tp in temp_parcels
                     if not tp.get('_is_ghost_sliver', False)]
    if _ghost_filtered_t1 > 0:
        try:
            import streamlit as _st_e04a
            _st_e04a.caption(
                f"🔧 _allocate_three_tier_v1 已排除 {_ghost_filtered_t1} 筆 ghost sliver"
            )
        except Exception:
            pass
    buildables = [tp for tp in temp_parcels
                   if tp.get('街廓分類', '') in BUILDABLE_CATS]
    publics = [tp for tp in temp_parcels
                if tp.get('街廓分類', '') in PUBLIC_CATS]

    def _avg_price(ri_label):
        sumv = 0.0; suma = 0.0
        for b in buildables:
            if b.get('所屬街廓', '') != ri_label:
                continue
            a = float(b.get('幾何面積_m2', b.get('面積_m2', 0)) or 0)
            p = float(pre_price_by_zone.get(b.get('重劃前地價區段', ''), 0) or 0)
            sumv += a * p; suma += a
        return (sumv / suma) if suma > 0 else 0.0

    avg_prices = {b.get('所屬街廓', ''): _avg_price(b.get('所屬街廓', ''))
                   for b in buildables}
    poly_cache = {}
    for tp in temp_parcels:
        cs = tp.get('polygon_coords') or []
    ...
    # ─── Tier 1：同歸戶 + 鄰接距 < 1m ───
    for pub in publics:
        pub_gid = ownership_map.get(pub.get('原地號', ''), '')
        if not pub_gid:
            try:
                pub_gid = _resolve_ownership(pub.get('原地號', ''), ownership_map)
            except Exception:
                pub_gid = ''
        if not pub_gid:
            continue
        pub_poly = poly_cache.get(id(pub))
        if pub_poly is None:
            continue
        target_bp = None
        for b in buildables:
            if ownership_map.get(b.get('原地號', ''), '') != pub_gid:
                continue
            b_poly = poly_cache.get(id(b))
            if b_poly is None:
                continue
            try:
                if pub_poly.distance(b_poly) < 1.0:
                    target_bp = b
                    break
            except Exception:
                continue
        if target_bp is None:
            continue
        ri_lbl = target_bp.get('所屬街廓', '')
        a_orig = float(pub.get('幾何面積_m2', pub.get('面積_m2', 0)) or 0)
        p_origin = float(pre_price_by_zone.get(
            pub.get('重劃前地價區段', ''), 0) or 0)
        p_target = avg_prices.get(ri_lbl, 0)
        a_prime = (a_orig * p_origin / p_target
                    if p_target > 0 and p_origin > 0 else a_orig)
        if _TIER1_APRIME_INJECT_ENABLED:
            # W-F：實際把同歸戶公設地 a' 併入建地 a 累加器（面積_m2）
            target_bp['面積_m2'] = round(
                float(target_bp.get('面積_m2', 0) or 0) + a_prime, 2)
            target_bp.setdefault('_tier1_merged_pubs', []).append({
                'pub_id': pub.get('原地號', ''),
                'a_orig': round(a_orig, 2),
                'a_prime': round(a_prime, 2),
            })
        # W-C：a_prime 已算出但不注入；仍認列配對 → consumed（不落入 Tier 2、維持公設地）
        consumed_pub_ids.add(id(pub))
        tier1_log.append({
            'pub_id': pub.get('原地號', ''),
            'pub_暫編': pub.get('暫編地號', ''),
            'gid': pub_gid,
            'target_bp': target_bp.get('暫編地號', ''),
            'target_ri': ri_lbl,
            'a_orig': round(a_orig, 2),
            'a_prime': round(a_prime, 2),
            '狀態': ('已併入建地 a' if _TIER1_APRIME_INJECT_ENABLED
                     else '延 W-F（未注入；建地 a=分攤登記）'),
        })
    pending_pubs = [p for p in publics if id(p) not in consumed_pub_ids]
    return {
        'tier1_log': tier1_log,
        'updated_parcels': temp_parcels,
        '_tier2_pending_pubs': pending_pubs,
        'cash_compensation': cash,
        'tier1_deferred': not _TIER1_APRIME_INJECT_ENABLED,
    }


def _allocate_tier2_tier3_geometric(g_rows: list,
                                      pending_pubs: list,
                                      classified_blocks: list,
                                      temp_parcels: list,
                                      ownership_map: dict,
                                      pre_price_by_zone: dict,
                                      min_area_by_block: dict,
                                      legal_min_width: float = 3.5) -> dict:
    """
    🚨 Patch D-3：Tier 2 + Tier 3 之幾何處理（Step G 後執行）
    Tier 2: 對每筆 pending pub，找剩餘抵費地最大之 Ri，從中切走 a'
    Tier 3: 殘餘抵費地 < min_area 或寬度 < legal_min_width → 平均增配 G
    """
    from shapely.geometry import Polygon as _P_d3b
    BUILDABLE_CATS = {'住宅區', '商業區'}
    tier2_log = []; tier3_log = []; cash = []
    if not g_rows:
        return {'tier2_log': [], 'tier3_log': [], 'cash_compensation': []}
    # 🚨 Patch E-0.4：排除 ghost sliver（_is_ghost_sliver=True 不參與分配計算）
    _ghost_filtered_t23 = sum(1 for tp in (temp_parcels or [])
                                if tp.get('_is_ghost_sliver', False))
    temp_parcels = [tp for tp in (temp_parcels or [])
                     if not tp.get('_is_ghost_sliver', False)]
    if _ghost_filtered_t23 > 0:
        try:
            import streamlit as _st_e04b
            _st_e04b.caption(
                f"🔧 _allocate_tier2_tier3_geometric 已排除 {_ghost_filtered_t23} 筆 ghost sliver"
            )
        except Exception:
            pass
    buildables_in_temp = [tp for tp in (temp_parcels or [])
                            if tp.get('街廓分類', '') in BUILDABLE_CATS]

    def _avg_price(ri_label):
        sumv = 0.0; suma = 0.0
        for b in buildables_in_temp:
            if b.get('所屬街廓', '') != ri_label:
                continue
            a = float(b.get('幾何面積_m2', b.get('面積_m2', 0)) or 0)
            p = float(pre_price_by_zone.get(b.get('重劃前地價區段', ''), 0) or 0)
    ...
    for r in g_rows:
        ...
        try:
            ...
            continue
        ri_lbl = r.get('所屬街廓', '')
        middle_offset_by_ri.setdefault(ri_lbl, []).append((r, p))
    buildable_ris = set()
    for b in classified_blocks or []:
        if b.get('category', '') in BUILDABLE_CATS:
            buildable_ris.add(b.get('label', ''))

    # ── Tier 2 ──
    for pub in pending_pubs or []:
        a_orig = float(pub.get('幾何面積_m2', pub.get('面積_m2', 0)) or 0)
        if a_orig <= 0:
            continue
        pub_gid = ownership_map.get(pub.get('原地號', ''), '')
        if not pub_gid:
            try:
                ...
                pub_gid = ''
        ri_capacities = []
        for ri_lbl, offset_list in middle_offset_by_ri.items():
            if ri_lbl not in buildable_ris:
                continue
            total_cap = sum(p.area for _, p in offset_list)
            if total_cap > 0:
                ri_capacities.append((ri_lbl, total_cap, offset_list))
        ri_capacities.sort(key=lambda x: -x[1])
        absorbed = False
        for ri_lbl, total_cap, offset_list in ri_capacities:
            min_area_ri = float(min_area_by_block.get(ri_lbl, 0) or 0)
            p_origin = float(pre_price_by_zone.get(
                pub.get('重劃前地價區段', ''), 0) or 0)
            p_target = _avg_price(ri_lbl)
            a_prime = (a_orig * p_origin / p_target
                        if p_target > 0 and p_origin > 0 else a_orig)
            offset_list.sort(key=lambda x: -x[1].area)
            largest_row, largest_poly = offset_list[0]
            if largest_poly.area < a_prime + min_area_ri:
                continue
            try:
                near_pt = (float(pub.get('centroid_x', 0)),
                            float(pub.get('centroid_y', 0)))
            except Exception:
                near_pt = None
            cut_geom, residual = _geometric_carve_from_side(
                largest_poly, a_prime, near_point=near_pt)
            if cut_geom is None or cut_geom.is_empty:
                continue
            try:
                if hasattr(residual, 'exterior'):
                    largest_row['cut_coords'] = list(residual.exterior.coords)
                    largest_row['幾何面積(㎡)'] = round(float(residual.area), 2)
                    largest_row['G(㎡)'] = round(float(residual.area), 2)
                offset_list[0] = (largest_row, residual)
            except Exception:
                continue
            try:
                cut_coords_new = (list(cut_geom.exterior.coords)
                                   if hasattr(cut_geom, 'exterior') else [])
            except Exception:
                cut_coords_new = []
            g_rows.append({
                '暫編地號': f'{ri_lbl}-Tier2-{pub.get("原地號", "")}',
                '原地號': pub.get('原地號', ''),
                '所屬街廓': ri_lbl,
                '重劃前區段': pub.get('重劃前地價區段', ''),
                'a 面積(㎡)': round(a_orig, 2),
                '街廓面積(㎡)': 0.0,
                '正面長度(m)': 0.0,
                '平均深度(m)': 0.0,
                ...
                '街角地': '—', '第1筆街角': '—', '街角側別': '—',
                'F(m)': 0.0, 'W(m)': 0.0, 'Rw(%)': 0.0, 'S(m)': 0.0,
                'G(㎡)': round(float(cut_geom.area), 2),
                '幾何面積(㎡)': round(float(cut_geom.area), 2),
                '累積S(m)': 0.0,
                '推進側別': '🟢 Tier2 跨街廓配地',
                'cut_coords': cut_coords_new,
                '解法': f'Patch D-3 Tier2: 跨街廓切片配地 a={a_orig:.1f}㎡ → a\'={a_prime:.1f}㎡',
                '迭代次數': 0, '是否收斂': '—',
                '負擔比率': round(1.0 - a_prime / a_orig, 4) if a_orig > 0 else 0.0,
            })
            tier2_log.append({
                'pub_id': pub.get('原地號', ''),
                'gid': pub_gid,
                ...
                'a_orig': round(a_orig, 2),
                'a_prime': round(a_prime, 2),
                'carved_area': round(float(cut_geom.area), 2),
            })
            absorbed = True
            break
        if not absorbed:
            cash.append({
                ...
                '所屬街廓': pub.get('所屬街廓', ''),
                '面積(㎡)': round(a_orig, 2),
                '狀態': 'Tier 2 失敗（各 Ri 抵費地不足切片）→ 建議現金補償',
            })

    # ── Tier 3：W-C 裁示① 停用（停止灌 G）──
    #   原 Tier3 把中間剩餘小片（size-gated）與 _treat_as_offset 殘片的面積，平均加進
    #   可建築 ri_buildable_rows 的 G——且只灌 G、不灌幾何 → 超額配地 + 白縫
    #   （實證：含 Tier3 的 R1/R3/R6 gap 精準 = ΣTier3 量；無 Tier3 的 R2/R4/R5 gap≈0）。
    #   裁示①：可建築 G = 附件二公式 G。小片 / treat 片不再 zero、不灌 G，回歸由
    #   offset_land（blk_poly − allocated，Task D-3 約 line 15167）自動畫成抵費地併入池。
    #   「折小片進鄰地」之調配（給可建築且 cut_coords 幾何同步長大）留 W-E/W-F。
    return {
        'tier2_log': tier2_log,
        'tier3_log': tier3_log,
        'cash_compensation': cash,
...
def calc_a_prime(area_origin: float, price_origin: float,
                 price_target: float, area_existing_target: float = 0.0) -> dict:
    """
    跨街廓調配面積（a' 公式）— 104 手冊 P.103 / P.121
      a' = 原街廓面積 × (P1 ÷ P2) + 目的街廓原有面積
      P1 = 原街廓重劃前宗地單價（元/㎡）
      P2 = 目的街廓重劃前宗地平均單價（元/㎡）

    參數：
      area_origin          原街廓需調出之面積 a (㎡)
      price_origin         原街廓重劃前宗地單價 P1 (元/㎡)
      price_target         目的街廓重劃前宗地單價 P2 (元/㎡)
      area_existing_target 目的街廓同一歸戶原已擁有之面積 a₂ (㎡；可為 0)

    回傳：
      {'a_prime': 換算後合併計算之等值面積, 'ratio': P1/P2, 'valid': bool}
    """
    a = float(area_origin or 0.0)
    p1 = float(price_origin or 0.0)
    p2 = float(price_target or 0.0)
    a2 = float(area_existing_target or 0.0)
    if p2 <= 0 or a <= 0:
        return {'a_prime': round(a + a2, 4), 'ratio': 0.0, 'valid': False}
    ratio = p1 / p2
    a_prime = a * ratio + a2
    return {'a_prime': round(a_prime, 4), 'ratio': round(ratio, 6), 'valid': True}
    ...
    # ===== Tab: 街廓互動分析 =====
        ...
        if parcel_polys:
            ...
            try:
                ...
                # 🚨 Patch D-3：三層調配 — Tier 1（同 Ri 同歸戶 → 修改 a 值）
                # 取代舊 _premerge_public_land_to_a（squeeze cap 版）+ Hotfix Fix E
                try:
                    _alloc_min_area_by_block = {}
                    for _b_alloc in classified_blocks or []:
                        _lbl_a = _b_alloc.get('label', '')
                        _cat_a = _b_alloc.get('category', '')
                        _sb_a = sb_rows_by_label.get(_lbl_a, {}) if 'sb_rows_by_label' in dir() else {}
                        _fw_a = float(_sb_a.get('正面路寬(m)', 0.0) or 0.0)
                        try:
                            _info_a = get_min_lot_size(_cat_a, _fw_a)
                            # 🚨 Patch E-1 §6：min_area 改用「Ri 實際分配深度」
                            _alloc_depth_a = float(_sb_a.get('街廓分配深度(m)', 0.0) or 0.0)
                            if _alloc_depth_a > 0 and _info_a.get('min_width', 0) > 0:
                                _cutoff_a = (_safe_num_e1(_sb_a.get('【左】截角(㎡)'))
                                             + _safe_num_e1(_sb_a.get('【右】截角(㎡)')))
                                _alloc_min_area_by_block[_lbl_a] = round(
                                    max(float(_info_a['min_width']) * _alloc_depth_a
                                        - _cutoff_a, 0.0), 2)
                            else:
                                _alloc_min_area_by_block[_lbl_a] = float(
                                    _info_a.get('min_area', 0.0) or 0.0)
                        except Exception:
                            _alloc_min_area_by_block[_lbl_a] = 0.0

                    _alloc_result = _allocate_three_tier_v1(
                        temp_parcels=temp_parcels,
                        classified_blocks=classified_blocks,
                        ownership_map=st.session_state.get(
                            't8_ownership_map', {}) or {},
                        pre_price_by_zone={
                            r.get('區段', ''): float(r.get('單價(元/㎡)', 0) or 0)
                            for r in (st.session_state.get('pre_zone_results',
                                                            []) or [])
                        },
                        min_area_by_block=_alloc_min_area_by_block,
                        legal_min_width=3.5,
                        burden_factor=0.55,
                    )
                    temp_parcels = _alloc_result['updated_parcels']
                    st.session_state['f3_temp_parcels'] = temp_parcels
                    st.session_state['f3_tier1_log'] = _alloc_result['tier1_log']
                    _deferred_t1 = bool(_alloc_result.get('tier1_deferred', False))
                    st.session_state['f3_tier1_deferred'] = _deferred_t1
                    st.session_state['f3_tier2_pending_pubs'] = (
                        _alloc_result.get('_tier2_pending_pubs', []) or []
                    )
                    st.session_state['f3_alloc_min_area_by_block'] = _alloc_min_area_by_block
                    # cash compensation 候選暫存（後續 Tier 2/3 會繼續處理）
                    st.session_state['f3_cash_compensation_list'] = (
                        _alloc_result.get('cash_compensation', []) or []
                    )
                    # 統計顯示
                    _n_t1 = len(_alloc_result.get('tier1_log', []) or [])
                    _n_pending = len(_alloc_result.get(
                        '_tier2_pending_pubs', []) or [])
                    _t1_area = sum(float(t.get('a_prime', 0) or 0)
                                    for t in (_alloc_result.get(
                                        'tier1_log', []) or []))
                    if _n_t1 > 0 or _n_pending > 0:
                        _msg_pre = ""
                        if _n_t1 > 0:
                            if _deferred_t1:
                                _msg_pre += (
                                    f"⏸️ **W-C 裁示：Tier 1 同歸戶公設地→建地補償延至 W-F**："
                                    f"{_n_t1} 筆公設地（a' 合計 **{_t1_area:,.2f} ㎡**）"
                                    f"暫不注入；W-C 建地 a = 分攤登記面積，"
                                    f"公設地維持為公設地、不配"
                                )
                            else:
                                _msg_pre += (
                                    f"✅ **Patch D-3 Tier 1（同 Ri 同歸戶）**："
                                    f"{_n_t1} 筆公設地已併入建地 a 值"
                                    f"（共 **{_t1_area:,.2f} ㎡**），"
                                    f"後續 G 迭代將自然反映"
                                )
                        if _n_pending > 0:
                            if _msg_pre:
                                _msg_pre += "\n\n"
                            _msg_pre += (
                                f"⏳ **Tier 2 待處理**：{_n_pending} 筆"
                                f"跨 Ri 孤立公設地將於 Step G 完成後做幾何切片配地"
                            )
                        st.info(_msg_pre)
                        if _n_t1 > 0:
                            with st.expander(
                                (f"🔎 Tier 1 同歸戶配對（{_n_t1} 筆，延 W-F 不注入）"
                                 if _deferred_t1
                                 else f"🔎 Tier 1 整併明細（{_n_t1} 筆）"),
                                expanded=False
                            ):
                                import pandas as _pd_pre
                                st.dataframe(
                                    _pd_pre.DataFrame(_alloc_result['tier1_log']),
                                    use_container_width=True, hide_index=True
                                )
                except Exception as _ePM:
                    st.warning(f"⚠️ 公設三層調配（Tier 1）失敗（不影響後續流程）：{str(_ePM)}")
...
""")
                ...
                if _btn_clicked or _auto_recalc:
                    ...
                    # 🚨 Patch D-3：Tier 2（跨 Ri 切片）+ Tier 3（殘餘抵費地平均增配）
                    # 取代舊 _consolidate_offset_polygons（Patch B-3）
                    try:
                        _min_area_by_block_for_t23 = {}
                        for _b_consol in classified_blocks:
                            _lbl_c = _b_consol.get('label', '')
                            _cat_c = _b_consol.get('category', '')
                            _sb_c = sb_rows_by_label.get(_lbl_c, {})
                            _fw_c = float(_sb_c.get('正面路寬(m)', 0.0) or 0.0)
                            _info_c = get_min_lot_size(_cat_c, _fw_c)
                            _min_area_by_block_for_t23[_lbl_c] = float(
                                _info_c.get('min_area', 0.0) or 0.0)
                        _t23_pending = (
                            st.session_state.get('f3_tier2_pending_pubs', []) or []
                        )
                        _t23_result = _allocate_tier2_tier3_geometric(
                            g_rows=g_rows,
                            pending_pubs=_t23_pending,
                            classified_blocks=classified_blocks,
                            temp_parcels=temp_parcels,
                            ownership_map=st.session_state.get(
                                't8_ownership_map', {}) or {},
                            ...
                            min_area_by_block=_min_area_by_block_for_t23,
                            legal_min_width=3.5,
                        )
                        st.session_state['f3_tier2_log'] = (
                            _t23_result.get('tier2_log', []) or [])
                        st.session_state['f3_tier3_log'] = (
                            _t23_result.get('tier3_log', []) or [])
                        # 將 Tier 2 失敗者併入 cash_compensation_list
                        _t23_cash = _t23_result.get('cash_compensation', []) or []
                        if _t23_cash:
                            _existing_cash = (
                                st.session_state.get(
                                    'f3_cash_compensation_list', []) or []
                            )
                            st.session_state['f3_cash_compensation_list'] = (
                                _existing_cash + _t23_cash
                            )
                        _n_t2 = len(_t23_result.get('tier2_log', []) or [])
                        _n_t3 = len(_t23_result.get('tier3_log', []) or [])
                        _t2_area = sum(float(t.get('carved_area', 0) or 0)
                                        for t in (_t23_result.get(
                                            'tier2_log', []) or []))
                        _t3_area = sum(float(t.get('distributed_area', 0) or 0)
                                        for t in (_t23_result.get(
                                            'tier3_log', []) or []))
                        if _n_t2 > 0:
                            st.success(
                                f"🟢 **Patch D-3 Tier 2 切片配地**："
                                f"{_n_t2} 筆跨街廓切片"
                                f"（共 **{_t2_area:,.2f} ㎡**）"
                            )
                        if _n_t3 > 0:
                            st.warning(
                                f"💧 **Patch D-3 Tier 3 平均擴張**："
                                f"{_n_t3} 個 Ri 共 **{_t3_area:,.2f} ㎡** 殘餘"
                                f"已平均增配至原位次地主"
                            )
                    except Exception as _eC:
                        st.warning(
                            f"⚠️ Patch D-3 Tier 2/3 幾何處理失敗"
                            f"（不影響原流程）：{_eC}"
                        )
                    ...
                    # 🚨 Patch D-6：三層調配明細（Tier 1 / Tier 2 / Tier 3）
                    _t1_log_d6 = st.session_state.get('f3_tier1_log', []) or []
                    _t2_log_d6 = st.session_state.get('f3_tier2_log', []) or []
                    _t3_log_d6 = st.session_state.get('f3_tier3_log', []) or []
                    if _t1_log_d6 or _t2_log_d6 or _t3_log_d6:
                        with st.expander(
                            f"💧 Patch D 三層調配明細："
                            f"Tier1 {len(_t1_log_d6)} 筆 / "
                            f"Tier2 {len(_t2_log_d6)} 筆 / "
                            f"Tier3 {len(_t3_log_d6)} 筆",
                            expanded=False
                        ):
                            if _t1_log_d6:
                                st.markdown(
                                    "##### Tier 1：同 Ri 同歸戶配對（W-C 延 W-F、未注入 a）"
                                    if st.session_state.get('f3_tier1_deferred', False)
                                    else "##### Tier 1：同 Ri 同歸戶 a 值併入"
                                )
                                st.dataframe(
                                    _pd.DataFrame(_t1_log_d6),
                                    use_container_width=True, hide_index=True
                                )
                            if _t2_log_d6:
                                st.markdown("##### Tier 2：跨 Ri 中間抵費地切片配給")
                                st.dataframe(
                                    _pd.DataFrame(_t2_log_d6),
                                    use_container_width=True, hide_index=True
                                )
                            if _t3_log_d6:
                                st.markdown("##### Tier 3：殘餘抵費地平均擴張")
                                st.dataframe(
                                    _pd.DataFrame(_t3_log_d6),
                                    use_container_width=True, hide_index=True
                                )
...
""")
            ...
            # ---------- 步驟 K：跨街廓調配（a' 計算） ----------
            if st.session_state.get('f3_J_cross_block_needed'):
                st.markdown("---")
                st.markdown("#### 🔁 步驟 K：跨街廓調配（a' 換算，讀取 Tab 8 歸戶資料）")
                st.markdown("""
**規則（市地重劃實施辦法 §30 第 3 款）：**
同一歸戶若於其他街廓尚有土地 → 將本街廓未達最小分配面積之地號，**調配至同歸戶之其他街廓**，
並以 **a' 公式** 換算等值面積：

> **a' = 原街廓面積 × (原街廓重劃前宗地單價 ÷ 目的街廓重劃前宗地單價) + 目的街廓原有面積**

**資料來源：**
- 原街廓 / 目的街廓「重劃前宗地單價」：Tab 5（地價區段分析）之重劃前區段單價
- 歸戶群組：Tab 8（土地歸戶）之 **t8_ownership_map**
""")

                _own_map = st.session_state.get('t8_ownership_map', {}) or {}
                _own_groups = st.session_state.get('t8_ownership_groups', {}) or {}

                # 🆕 任務三：Step K 進入前防呆 — 過濾出「目前 temp_parcels 中仍存在」之 cross-block 條目
                _current_temp_set = {tp.get('暫編地號') for tp in (temp_parcels or [])}
                _raw_cross = list(st.session_state.get('f3_J_cross_block_needed') or [])
                _filtered_cross = [item for item in _raw_cross
                                   if item.get('主要暫編地號') in _current_temp_set]
                _stale_count = len(_raw_cross) - len(_filtered_cross)
                if _stale_count > 0:
                    st.warning(
                        f"⚠️ 偵測到 {_stale_count} 筆「需跨街廓調配」之暫編地號已不在目前清單中"
                        f"（可能於步驟 C 已被剔除）。請回到**步驟 G** 重新執行 G 值迭代計算，"
                        f"再回到本步驟 K，以確保資料一致。"
                    )

                if not _own_map:
                    st.warning("⚠️ 尚未偵測到 Tab 8 歸戶資料。請先至 Tab 8（🏘️ 土地歸戶）匯入歸戶 Excel → 系統完成歸戶後會自動寫入 session_state。")
                else:
                    st.success(f"✅ 已讀取 Tab 8 歸戶資料：共 {len(_own_map)} 筆地號、{len(_own_groups)} 個歸戶群組")

                    # 準備 Tab 5 重劃前區段單價表
                    _pre_price_by_zone = {r['區段']: float(r.get('單價(元/㎡)', 0) or 0)
                                          for r in (st.session_state.get('pre_zone_results') or [])}

                    # 把「需跨街廓調配」地號對應到歸戶群組（只跑通過防呆篩選之條目）
                    _k_rows = []
                    _k_detail = []
                    for item in _filtered_cross:
                        _parent_no = item['原地號']
                        _primary = item['主要暫編地號']
                        _cur_g = float(item['目前G(㎡)'])
                        _min_a = float(item['最小分配面積(㎡)'])
                        _src_blk = item['所屬街廓']
                        _gid = _own_map.get(_parent_no, '')

                        # 該歸戶的所有地號
                        _grp_parcels = _own_groups.get(_gid, []) if _gid else []
                        # 找同歸戶但不同街廓的暫編地號（從 f3_J_merged）
                        _other_blocks = []
                        for mr in (st.session_state.get('f3_J_merged', []) or []):
                            if mr.get('原地號') in _grp_parcels and mr.get('原地號') != _parent_no:
                                _other_blocks.append(mr)

                        if not _gid:
                            _k_rows.append({
                                '原地號': _parent_no, '主要暫編地號': _primary,
                                '所屬街廓': _src_blk, '歸戶群組': '(查無)',
                                '目前G(㎡)': _cur_g, '最小分配面積(㎡)': _min_a,
                                '缺口(㎡)': round(max(0.0, _min_a - _cur_g), 2),
                                '建議目的街廓': '—', 'P1(元/㎡)': 0, 'P2(元/㎡)': 0,
                                'a\'(㎡)': 0.0, '狀態': '❌ 未找到歸戶資料 → 僅能現金補償',
                            })
                            continue

                        if not _other_blocks:
                            _k_rows.append({
                                '原地號': _parent_no, '主要暫編地號': _primary,
                                '所屬街廓': _src_blk, '歸戶群組': _gid,
                                '目前G(㎡)': _cur_g, '最小分配面積(㎡)': _min_a,
                                '缺口(㎡)': round(max(0.0, _min_a - _cur_g), 2),
                                '建議目的街廓': '—', 'P1(元/㎡)': 0, 'P2(元/㎡)': 0,
                                'a\'(㎡)': 0.0,
                                '狀態': '⚠️ 同歸戶無其他街廓土地 → 建議現金補償',
                            })
                            continue

                        # 取同歸戶目的街廓中「G 值最大者」作為合併目標
                        _target = max(_other_blocks, key=lambda x: float(x.get('合併後G(㎡)', x.get('G(㎡)', 0)) or 0))
                        _tgt_blk = _target.get('所屬街廓', '')
                        _src_zone = ''
                        _tgt_zone = ''
                        # 從 temp_parcels 查源/目的區段
                        for tp in (temp_parcels or []):
                            if tp.get('暫編地號') == _primary:
                                _src_zone = tp.get('重劃前地價區段', '')
                            if tp.get('暫編地號') == _target.get('暫編地號'):
                                _tgt_zone = tp.get('重劃前地價區段', '')
                        _p1 = _pre_price_by_zone.get(_src_zone, 0.0)
                        _p2 = _pre_price_by_zone.get(_tgt_zone, 0.0)
                        _area_existing = float(_target.get('合併後G(㎡)', _target.get('G(㎡)', 0)) or 0)

                        _res = calc_a_prime(
                            area_origin=_cur_g,
                            price_origin=_p1, price_target=_p2,
                            area_existing_target=_area_existing,
                        )

                        _status = '✅ 可調配' if _res['valid'] else '⚠️ P1/P2 未填（請檢查 Tab 5 區段單價）'
                        _k_rows.append({
                            '原地號': _parent_no, '主要暫編地號': _primary,
                            '所屬街廓': _src_blk, '歸戶群組': _gid,
                            '目前G(㎡)': round(_cur_g, 2),
                            '最小分配面積(㎡)': round(_min_a, 2),
                            '缺口(㎡)': round(max(0.0, _min_a - _cur_g), 2),
                            '建議目的街廓': _tgt_blk,
                            'P1(元/㎡)': round(_p1), 'P2(元/㎡)': round(_p2),
                            'a\'(㎡)': _res['a_prime'], '狀態': _status,
                        })
                        _k_detail.append({
                            '原地號': _parent_no, '主要暫編地號': _primary,
                            '原街廓(區段)': f"{_src_blk}({_src_zone})",
                            '目的街廓(區段)': f"{_tgt_blk}({_tgt_zone})",
                            '原街廓需調出面積a(㎡)': round(_cur_g, 2),
                            '地價比率P1/P2': _res['ratio'],
                            '目的街廓原有面積a₂(㎡)': round(_area_existing, 2),
                            'a\'合併後面積(㎡)': _res['a_prime'],
                        })

                    if _k_rows:
                        st.markdown("##### 📊 跨街廓調配建議")
                        _df_k = _pd.DataFrame(_k_rows)
                        st.dataframe(_df_k.style.format({
                            '目前G(㎡)': '{:.2f}', '最小分配面積(㎡)': '{:.2f}',
                            '缺口(㎡)': '{:.2f}', 'a\'(㎡)': '{:.2f}',
                            'P1(元/㎡)': '{:,.0f}', 'P2(元/㎡)': '{:,.0f}',
                        }), use_container_width=True, hide_index=True)
                        ...
                        if _k_detail:
                            with st.expander("🔎 a' 計算明細（P1/P2 與換算過程）"):
                                st.dataframe(_pd.DataFrame(_k_detail),
                                             use_container_width=True, hide_index=True)

                        st.session_state['f3_K_reallocation'] = _k_rows
