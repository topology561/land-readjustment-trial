...
def run_all_cases():
    ...
    # 案1（KL 例，D-1 8/8 之錨）：w=[12,24,6] 雙側等權 → J(1)=R(12)+R(30)=180、
    #   J(2)=R(36)+R(6)=150 → k*=1（J 精確值鎖死）
    r = slot([12, 24, 6], _side(True), _side(True))
    jb = {t["k"]: round(t["J"], 4) for t in r["table"]}
    assert jb == {1: 180.0, 2: 150.0}, f"案1 J 表應 {{1:180.0, 2:150.0}}，實得 {jb}"
    ...
    #   J(1)=100+R(10)=171.1 > J(2)=100+R(5)=144.0 → k*=1
    r = slot([40, 5, 5], _side(True), _side(True))
    jb = {t["k"]: round(t["J"], 4) for t in r["table"]}
    assert jb == {1: 171.1, 2: 144.0}, f"案6 J 表應 {{1:171.1, 2:144.0}}，實得 {jb}"
    ...
    #   J(1)=2·R(4)+R(8)=136.7、J(2)=2·R(8)+R(4)=160.0 → k*=2（餵大權側；
    #   reviewer NOTE：直接鎖 k 值不依賴 J 大小關係）
    r = slot([4, 4, 4], _side(True, F=2.0, l1=1.0), _side(True, F=1.0, l1=1.0))
    jb = {t["k"]: round(t["J"], 4) for t in r["table"]}
    assert jb == {1: 136.7, 2: 160.0}, f"案8 J 表應 {{1:136.7, 2:160.0}}，實得 {jb}"
...
if __name__ == "__main__":
    n = run_all_cases()
    print(f"✅ W-D.2 滑池槽 golden {n}/{n} 通過（KL 例 J=180/150、E1-E7 全過）")
