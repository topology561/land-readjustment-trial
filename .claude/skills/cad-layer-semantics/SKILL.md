...
- **`_BL_EQUIV_EPS_M = 1e-4 m（0.1mm）`**，依據＝**DXF 座標精度**（非繪圖意圖）：
  · 下限：實測同線兩段之垂距差 **R1 5.169e-07／R4 5.795e-06** ⇒ 留 ~17 倍餘裕。
    ⚠️ **門檻不得取 1e-6**——`R4 之 5.795e-06 > 1e-6`，取之則 R4 落出①、誤觸 ② raise
    （夾具已釘此格）。
  · 上限：KL 令不得超過 **0.005m**（法定 2dp 末位之半）；本值低其 50 倍。
  · 對照：較舊用之 `1.0m` 繪圖容差**嚴 1 萬倍**。
- 🔴 **舊①之洞（KL 指認）**：屁股線畫成兩段而有 **0.5m 錯位**（真實圖常見）會被判「同一條線」
  → 靜默取最小 handle → **深度平移 0.5m 且無任何訊號**；而最淺街廓 R4(33.1046) vs
  R1(33.1461) 僅差 **0.0415m**、MinA／½ 線掛其上 ⇒ 該洞為決勝差距之 **12 倍**。
  且**①先於②判定** ⇒ ②之 raise 永遠不觸發。**修主路徑漏支路＝沒修**（同下方血訓）。
- **golden＝UC9898 R1／R4**（皆 `equivalent_n == 2`）。
  夾具：`verify/fixture_baseline_candidates.py`（三分支＋0.5m 錯位須轉②＋反例自證＋兩 golden）。
