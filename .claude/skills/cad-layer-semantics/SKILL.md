...
| # | 條件 | 處置 |
|---|---|---|
| ① | ≥2 且**彼此共線**（方位 mod180 差 ≤ ang_tol ∧ 互垂距 ≤ perp_tol） | **幾何等價** ⇒ 取確定性規則（**最小 DXF handle**）＋記 `_match.equivalent_n`；**不進 `_ambig`** |
| ② | ≥2、**不共線**、勝差 ≤ perp_tol | **進 `_ambig`** → `CadBindingAmbiguity` → UI 確認頁。**禁靜默選** |
| ③ | ≥2、不共線、勝差 > perp_tol | 採垂距最小者 |

- **golden＝UC9898 之 R1**（`equivalent_n == 2`·BL#1/BL#3 共線）。
  夾具：`verify/fixture_baseline_candidates.py`（三分支＋反例自證＋golden）。
