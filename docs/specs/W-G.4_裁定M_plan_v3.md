...
- **W-9 成本：已實測·疑慮解除**（施工前置已辦·`scratchpad/time_stepg.py`·3 次取平均）：

  | 項 | 壁鐘 |
  |---|---|
  | `harvest()` | 1.87s |
  | prep（`build_pipeline`＋`build_param_table`＋`build_build_parcels`） | 0.58s |
  | `run_corner_pk` | 0.01s |
  | **`run_step_g`** | **0.21s**（`0.22／0.21／0.21`） |

  ⇒ 最壞成本 P-D(+1)＋E-3(+6) × 雙情境 ＝ **+14 次 ≈ +3.0s**
  （對照 `run_all` 全跑之數分鐘 ⇒ **增幅 <2%**）。
  **S-8 停機條件（翻倍以上）確定不會觸發**；E-3 迭代上限 6 無須壓縮。
...
| ~~S-8~~ | ~~`run_all` 壁鐘因 P-D/E-3 翻倍以上（W-9）~~ | **已實測解除**：`run_step_g` 0.21s ⇒ +14 次 ≈ +3.0s（增幅 <2%）·**確定不觸發** |
