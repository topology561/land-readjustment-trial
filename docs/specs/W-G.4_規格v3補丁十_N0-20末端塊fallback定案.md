...
- **條件2（該側有未臨正街）**：該側「未臨正街半平面」面積 `> ε`（＝補丁七 §一.1 **判別語意**）·**side-parametrized**（claude.ai 定稿·W3）：
  - `side==left`：`block∩{s<0}`（rel p1·s(p1)=0）。
  - `side==right`：`block∩{s>s(p2)}`（`s(p2)`＝p2 於 ALLOC 切線軸相對 p1 之 s 座標）。
  末端邊∥ALLOCLINE ⇒ 該側半平面 ≈0 ⇒ 無·跳過。
