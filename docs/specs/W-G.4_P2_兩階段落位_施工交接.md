...
| `_adv_final` 產生 | `:686`／`:727` | `app.py:15818`／`:15842`（**雙**產生點·見下） |
...
- ⚠️ **W-10 於 app 側同樣適用（表格補正·claude.ai 2026-07-23 獨立坐實）**：app 之 `_adv_final` 亦為
  **兩個**產生點——`app.py:15818`（degenerate／`N≤1` 分支）與 `app.py:15842`（正常分支）；
  中間 `:15821` 是 `_adv_base`（選槽 base·非 final），**勿誤認為第三產生點**。
  插入點 `:15843` 本就在 if/else **匯流之後** ⇒ **W-10 於 app 側自動滿足**，照本文施工不會漏接 degenerate 分支。
