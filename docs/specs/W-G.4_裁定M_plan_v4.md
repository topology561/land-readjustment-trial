...
- `G007 = {628-20, 628-30, 628-45}`；`G010 = {628-40, 628-43, 628-44}`；`G022 = {628-7}`。
- **R2左 池∅ 之第三員守衛**：G010 有第三員 `628-44`——實查 `628-44(1)` 在 **RD1（道路·非可建築）**、
  不入 `build_parcels` ⇒ **非合法 M-2 源** ⇒ R2左 池仍 ∅（僅 628-40(1)/628-43(1)·皆 R2 同塊）。
- **R5左 池∅**：G022 僅 `628-7`；`628-7(1)@R6` G=306.92 ≥ MinA[R6]=156.06 **已達標** ⇒ 非「未達地」⇒ ∅。
- **R3右 源合法**：`628-45(1)@R5`(G52.19)／`628-30(1)@R5`(G15.74)／`628-30(2)@R2`(G89.98) 皆
  **其他可建築街廓 ∧ < MinA** ⇒ 合法源（Σa=270.00·gid=G007=corner 628-45(2) 同歸戶·✓）。
...
>
> **本節 P-H 之重烤全面凍結，未經 KL 解凍不得執行。**（P-F／P-G 已收官部分不受影響。）
>
> **事由**：裁定 N 判分配近端起算點寫死 `s=0`（`corner_pt = FRONT_LINE p1`）為錯、
> 應改為街廓 s 域真下界 `s_min`（遠端 `_oblique_s_max` 已正確·不動）⇒ **分配幾何將改變**。
> 現在烤＝把**錯誤起算點永久定為基準**。
>
...
2. ~~**`W-F F.4` 3.5m E2「上界 7 < 需求 9」**是否本波處理~~ ⇒ **P-G 實測已解·本項可撤**：
   上界 **7→9**、`_feas` **真窮舉判「可行」並輸出建構指派**（`M_0725_PG_capdecomp.log`）。
   `W-F F.4` 現行紅之成因**已換**為「F.0 raise 之存在性守衛級聯」（待 P-H 重錨），
   **非** E2 結構性不可行。詳 `W-G.4_裁定M_PG_容量拆解重跑.md` §五。
