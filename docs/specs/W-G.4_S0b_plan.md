...
| 2 | ~~小數 2→≥4 位~~ **🔴 廢止（KL 2026-07-17）** | **(i) N0-16 溶解前提**（匯出屬 ④ 接線層·不負 0.01 級閘之舉證）＋**(ii) N0-18：法定成果面積本即 2dp**（報表 4dp 反與 N0-18 相衝）。原立意誤設；且源 g_rows 本即 2dp（`stepg:276/277`）→ `round(2dp,4)`＝no-op。**引擎 schema 改（增未捨入欄）：駁回。** → 數值欄**回 2dp**、A2 註記刪 4dp 字樣 |
| 3 | 情境標記 0m/3.5m | `export_legal_excel` 增參數 `scenario_tag`；Sheet-1 增 `'情境'` 欄（或檔名/分頁標）·兩情境不撞名 |
| 4 | 階段標記 ＋ **🆕 `G_原值(㎡)` 欄**（KL 2026-07-17 修正） | 增 `'階段'` 欄·避免誤判。**⚠️ 標記須為 `trunk A′（Step G＋步驟J 就地改寫後）`**——Phase 7 之 `f3_G_values` 已被 `app.py:16717-16732`（Patch B-2 寬度驗證）就地壓 G。併增 **`G_原值(㎡)`**（取 `_G_before_width_violation`）＋**`增減(㎡)` 改以原值計** |
...
| ③④ | **🔴 原文數值錨已廢·改恆等式（KL 2026-07-17 落章）**：**`Δ池 ＋ Σ(Δ宗幾何) == net_leak`** | 容差 **`(宗數＋片數＋1)×0.005`**（三項皆 2dp 捨入半量子·**禁用 0.01**＝N0-17-a 錯配）｜**破＝第五源→停機** | 錨為 pre-S0c → 隨「錨只重定一次」全數作廢。**實測 F.4 終態 12/12 至 ±0.04＝無第五源**；F.0 則 ±0.59（反證 net_leak 為 F.4 量·**比對階段須為錨之定義階段**） |
