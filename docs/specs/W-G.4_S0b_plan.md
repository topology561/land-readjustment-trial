...
### 5.2 鋪滿閘（新硬閘·`|Σ全片幾何 − 街廓面積| ≤ 0.01`）

- 立於 T2 helper 內（§1.5）＋ run_all 逐街廓×逐情境。附：**任兩片交集 ≤ 1e-6**（不重疊·數值極限）、**池帳−池幾何 ≤ 0.01**（本病直接量測·§閘③）。
- 落點：`verify/run_verification.py`（F.2/F.3/F.4 各段·於守恆閘鄰近新增 `results.append`）＋ stepg/app T2 helper 內建自檢。
- 依據＝幾何精度上限。

### 5.3 守恆閘收緊（真閘·6.0/1.0 → 0.01）

- **全區**（`cons_resid < 6.0` → `< 0.01`）：`verify/run_verification.py:889`（F.2·`6.0`）、`951`（F.3·`6`·AND `verdict_all_green`）、`1009`（F.4·`6`·AND `verdict_all_green`）。**含 952/1010 之 fail-detail 重複條件同步改**。label 文案 `<6㎡` → `<0.01㎡`。
- **逐街廓**（`abs(_resid) < 1.0` → `< 0.01`）：`verify/stepg_pipeline.py:813`（→ 846 raise 觸發）＋`app.py:15341`（→ 15375 st.error）。
...
1. **helper `_pool_strips_for_block` 立**（§1.5）＋鋪滿自檢 → py_compile。
2. **T2-#1 stepg** 接 helper → 單街廓 REPL 驗鋪滿/不重疊/池帳−池幾何 綠。
3. **T2-#2 app** 同構（byte 對映 stepg）。
...
9. **守恆閘收緊**（§5.3·6.0/1.0→0.01·**此時方改**·否則現值立紅）→ run_all 綠。
