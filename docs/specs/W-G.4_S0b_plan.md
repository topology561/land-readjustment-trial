# W-G.4 S0b 施工 plan（幾何漏歸零·修漏 T1/T2/T3＋補償碼拆＋三閘＋乙匯出＋甲-2）

> 依據：`docs/specs/W-G.4_規格_v3.md` §N3-0（唯一施工依據）＋ `W-G.4_CC交辦_v3.md`（丙時序第 1 步）。
> 基準 HEAD：`100cfa3`（規格/交辦 v3 已入倉·病灶原封未動）。
> 範圍：**僅 S0b**（修幾何漏）。**不動** §N1 buffer 幾何化、§N2 街角、§N3 targeting、§N5 下游——彼等為 S1~S5，本 plan 一律不觸。
...
- **③ 池帳−池幾何 ≤ 0.01**（保留）。
- **Σ 式降為診斷輸出**（印殘差＋宗-宗重疊，供 S0c 驗收對照）。
...
- **守恆-幾何級**：`|union(宗+池) − 街廓| ≤ 0.01` ＝ **①' 覆蓋閘同一條**（勿重複立閘）。
- **守恆-帳幾何級（逐街廓）**：`|ΣG ＋ 池幾何 − 街廓| ≤ 宗數×0.005 ＋ 圍堵界`（圍堵界＝`(宗數−1)×0.005×分配深度`）：`verify/stepg_pipeline.py:813`（→846 raise）＋`app.py:15341`（→15375 st.error）。**S0c 後圍堵界歸零 → 收為 `宗數×0.005`。**
- **全區**（`cons_resid`）：`≤ Σ各街廓(宗數×0.005 ＋ 圍堵界)`：`verify/run_verification.py:889`（F.2·`6.0`）、`951`（F.3·`6`·AND `verdict_all_green`）、`1009`（F.4·`6`·AND `verdict_all_green`）。**含 952/1010 之 fail-detail 重複條件同步改**；label 文案同步。**S0c 後收為 `Σ宗數×0.005`。**
...
| 11 | E3 整形 `0.1` | `wf_f4:709`（`abs(new_area−old_area)>0.1` 整形守恆） | 真閘 | ⚠️ 待歸因（暫標·不得再以全綠作正確性根據·§N5/後波） |
...
| +補2 | `WEDGE_AREA_ANCHOR=5.30 ±0.05` | `verify/wf_f1.py:31/196`（R1 楔形量測錨·`量測為準`）／`wf_f4:58 F1_REVERIFY`（僅**身分**錨·不變） | 真閘 | 🔴 **T2 直接衝擊**：R1 net_leak +0.19(3.5m)/+0.17(0m) **> ±0.05 tol** → 楔形（R1-抵費地-2·5.30）吸漏後 area 增 → **196 之 ±0.05 硬檢先破**。**本波重烤時中心值 5.30 → 新量測值·並將 ±0.05（量測 wiggle·無原理依據）標殘餘定閘候選**（§9 step 8 前置）。身分錨 `TARGET_ANCHOR`/`F1_REVERIFY`（哪筆為標的）**不變**。 |
...
| # | 閘 | 判準 |
|---|---|---|
| ① | 業主宗 G **一字不變** | T2 不動切線/solve_G_binary·僅改池產生法 → 絕對閘·一格不符即停 |
| ② | 業主宗幾何位移 ≤0.01 | cut_coords 未動·應 0 |
| ③ | 池幾何逐街廓 **+= 已證 net_leak** | 3.5m R1+0.19/R2+0.17/R3+0.25/R4+0.02/R5+0.22/R6+0.17；0m R1+0.17/R2**+0.69**/R3+0.05/R4+0.12/R5+0.26/R6+0.17（±0.01） |
| ④ | 重算後 net_leak == 0 | 池帳−池幾何 ≤0.01（§閘③）全街廓×情境 |
| ⑤ | T1 新留 <1㎡ 片逐片可歸因 | 原被 `area>=1.0` 丟棄之末筆楔形·無不可歸因新增 |
...
## 9. 施工序（S0b 段內分段·每段閘綠再前進）

1. ✅**已完成** **helper `_pool_strips_for_block` 立**（§1.5·斜交切線座標）＋內建自檢（①' 覆蓋／②-池／**②-宗圍堵**／Σ 式診斷）＋T1 雙路 loud → py_compile。
2. ✅**已完成（覆蓋實證）** **T2-#1 stepg** 接 helper → 實測 `union(宗+池)−街廓 = 0.0000`、逐宗 slab 差額 0.0000、池側重疊 0。
3. ✅**已完成** **T2-#2 app** 同構（byte 對映 stepg）。
   → **§9 中斷點＝此處**（第四源停機·`ee5f844`）；KL 2026-07-16 裁定後**自 step 4 續行**（先補 ②-宗圍堵閘入 helper／harness）。
4. **T2-#3 wf_f1**＋**補償-1 拆**（§4.1）→ F.1 R1 REPL 綠。
5. **T2-#4 wf_f4**（`_reshape_block` 回傳 pool_polys）＋**補償-2 拆**（§4.2）→ E3 REPL 綠。
6. **T1** 四處（§2）＋**補償-3**（app:15509·§4.3）。
7. **乙 匯出升級**（§6·純 UI）。
8. **全譜系重烤**（f3 起·舊 baseline 依 PRE 凍存）→ **預測差量閘①~⑤＋鋪滿/池帳−池幾何/白縫 全綠**。
   - **前置·碼錨重定**：`WEDGE_AREA_ANCHOR`（wf_f1:31·5.30→新量測值·因 R1 楔形吸 +0.19 漏 > ±0.05 tol）須先隨重烤更新中心值，否則 196 之硬檢先破（此為預測差量閘③之碼常數對映·非 baseline 檔）。更新後 ±0.05 tol 標「待歸因」（量測 wiggle·§7 +補2）。**其餘 baseline 快照 CSV 依 PRE 凍存慣例重烤。**
9. **守恆閘收緊**（§5.3·**KL 2026-07-16 裁改·非 0.01**）：幾何級＝①' 覆蓋閘（同一條）；帳幾何級（逐街廓）→ `宗數×0.005 ＋ 圍堵界`；全區 `cons_resid` → `Σ各街廓(宗數×0.005 ＋ 圍堵界)`。**此時方改**（否則現值立紅）→ run_all 綠。**⚠️ 原文「6.0/1.0→0.01」已廢——照原文收緊必再停機一次。**
10. py_compile→grep 異動清單對照本 plan→**commit + push**（嚴格 `git rev-parse origin/main` 驗）。
11. 報告入倉 `docs/reports/W-G.4_S0b_修漏報告.md`·聊天僅 ping。
...
- **不動** solve_G_binary/_block_strip 之切帶邏輯（僅**新增** helper 呼叫 _block_strip 切池帶）。
