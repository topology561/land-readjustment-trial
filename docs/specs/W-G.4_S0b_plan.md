...
| **3.6** | 🆕 **三閘＋圍堵重驗**：①' 覆蓋 0.01／②-池 1e-6／**②-宗圍堵收 ≤1e-6**／守恆-帳幾何級（圍堵界歸零→`宗數×0.005`）／**Σ 診斷之縫與疊應歸 0**（不歸 0 即另有病·**停機**） | 🆕 **新增** |
...
**step 9 之收緊值（§5.3·⚠️ 原文「6.0/1.0→0.01」已廢——照原文收緊必再停機一次）**
- 幾何級＝①' 覆蓋閘（同一條·勿重複立閘）；帳幾何級（逐街廓）→ **`宗數×0.005`**；全區 `cons_resid` → **`Σ宗數×0.005`**（S0c 併入故圍堵界＝0）。**此時方改**（否則現值立紅）→ run_all 綠。
