...
### `§九-1`　逐閘（**報告入倉後量**·態 ＝ `86109af6b7b9f379dd60f84acb6c252e6dd76e95`）

| # | 受詞（**指令**） | 期／判準 | 實測 | 判 |
|---|---|---|---|---|
| `1` | `git show --numstat <各 commit>`（**主線與分支分列**） | 主線刪除欄**全 `0`**；分支之修批 `commit` 除外 | **主線** `5b39171c` ＝ `208`／**`0`**；`86109af6` ＝ `395`／`0`・`1152`／`0`・`1088`／`0`・`116`／`0`・`266`／`0`・`84`／`0`・`685`／**`0`** ⇒ 刪除欄**全 `0`**。🔒 **分支 ＝ 無**（工項二因停二未辦 ⇒ ⛔ 修批 `commit`·其「`deletions` 須 ＝ 改動處數」之款**無受詞**） | ✅ |
| `2` | 生產碼判法·母體**正面列舉 `34` 檔** | 主線逐區間 **`0`** 行；對照組[必非零] `≥ 1` | `5b39171c^..5b39171c` ＝ **`0`**；`86109af6^..86109af6` ＝ **`0`**；對照組 `17fd981^..17fd981` ＝ **`2`** ⇒ 器非紅。🔒 **分支之「須 `≥ 1` 行」⛔ 有受詞**（未開分支） | ✅ |
| `3` | `.md`／`.py` 之 `CR`（框 ＝ 倉內 blob·**取 bytes**·`git diff --name-only -z` ＋ `core.quotePath=false`） | `0` | 本批異動 **`8`** 檔，其 `.md`／`.py` 共 `3` 檔（`25062`／`33596`／`35762` B）`CR` 合計 ＝ **`0`**；判別力[必非零] `data/V6.dxf` `CR` ＝ **`12308`**／全長 **`88341`** B | ✅ |
| `4` | 四簿期末（**器之輸出**·⛔ 自擬單框） | **零鑄號** | 自誤 相異 **`393`**／MAX **`403`**／缺 `[106, 355, 356, 357]`｜`GB` `168`／`170`／缺 `[12, 87]`｜`VR` `80`／`95`／缺 `[73, 75]`｜`K-9` `28`／`29`／缺 `[]` ⇒ **與期初逐格相同** | ✅ |
| `5` | 坑之期末 ＋ `ALPHA_LABELS` ＋ `.md` 檔數自洽 | `87`／`87`·下一 `bj`；`numstat` 空；增量 ＝ 本批新增 `.md` 數 | **`87` 標籤／`87` 可解析**·末標籤 **`bi`** ⇒ 下一 **`bj`**（⛔ 鑄坑）；`ALPHA_LABELS` 載體之 `git diff --numstat` 輸出 ＝ **空**；追蹤 `.md` **`799` → `801`**（**＋`2`**）＝ 本批新增之 `.md` 檔數（單 `1` ＋ 報告 `1`）⇒ **自洽**；器二造：造甲 坑 `n` 定義處 `6 ≥ 1` ✅／造乙 人造坑 `0`／`0` ✅ | ✅ |
| `5′` | `wg9223_acceptance_audit.py <本單> <本報告>` | 命中 `0` 之字樣 ＝ `0` | 判定組（反引號 token）**`31`** 個 ⇒ 報告命中 `0` 者 ＝ **`0`** 個（`rc = 0`）；參考組 `3` 個中 `2` 個命中 `0`（**⛔ 作判**·係單之行文片段）。🔒 受詞係 `index` ⇒ **復跑前已 `git add`** | ✅ |
| `6` | 自限復驗 | 見右 | 生產碼 `34` 檔 blob 期初＝期末**相異 `0`**·`app.py` ＝ `4379108a4856714078c410ff4e6291ecfdf6d2c1`；判別力[必相異] ＝ 本批確有異動之 `docs/orders/W-G.9-300_…md`（期初 `cat-file -e` `rc = 128` ⇒ 不存在／期末 blob `e78cb6d8705c369a…`）⇒ **相異 `True`** ⇒ 該比對**確能偵出相異**；`baselines` **`298`** 檔·`a898f4e1bba8…` **未變**；`p3a` 對主線 `merge-base --is-ancestor` `rc = 1`；`WV_BAKE` ＝ `None`；`k6_step0_merge(` `2`／`rect_fits` `7`／`_blue_shadow_tri` `7` **期初同值** ⇒ `K-9-29`／`K-9-5-*`／`K-9-12` **⛔ 落地任何子項**；`GB-79`／`167`／`168`／`169`／`170` **⛔ 修**；四物證 ＋ 本批新增五檔之 blob **三態分列**（四物證 `T/T` 逐位未動；本批五檔 `F/T` **新入倉**）；對照組[必為零] 執行期組出之人造路徑期末存在 ＝ `False` | ✅ |
| `6′` | `git ls-remote origin 'refs/heads/verify/*'`（**二態分列**·⛔ 以其一充判） | 工項二**未辦** ⇒ `…-299-gb170` 命中 **`0`** | 該族共 **`20`** 個；`verify/W-G.9-299-gb170` ＝ **`0`**（⇒ **態「因停機未辦」**）；判別力[必存在] `verify/W-G.9-269-p3a` ＝ **`1`** | ✅ |
| `7` | `git ls-remote origin refs/heads/wip/s1-endpart`（`push` **後**） | 🔒 **出艙於回報·⛔ 入倉** | （其 `sha` 於本節自身落地前不存在·**結構上遞迴** ⇒ 逐字見聊天回報） | — |

### `§九-2`　本批之自捕（**續 `§六`**·第 `6` 則·於**收工閘**期間自捕·⛔ 頂替·🛑 ⛔ 鑄號）

| # | 形 | 徵候 | 根因 | 攔法 |
|---|---|---|---|---|
| `6` | 收工閘之量測腳本印出 `UnicodeDecodeError: 'cp950' codec can't decode byte 0xe9`（`Thread-152 (_readerthread)`），**而其後各格照常印出綠** | 例外落在**子行程之讀取執行緒**、⛔ 主流程 ⇒ **⛔ 中斷**、亦⛔ 使 `rc` 非零 | 判別力[必相異]之造對一**含 CJK 之路徑**跑 `git cat-file -e <BASE>:<path>`，其**預期即失敗**；`git` 之 `fatal:` 訊息含該路徑之 **UTF-8 bytes**，而 `subprocess(text=True)` 以 **`cp950`** 解 `stderr` ⇒ 擲例外**並丟棄該串流** | **以 bytes 重取**驗其未污染：`rc`（bytes 框）＝ **`128`** ＝ text 框之值；`stderr` 以 `cp950` 解 ⇒ `UnicodeDecodeError`（坐實）⇒ 🔒 **失敗之受詞係 `stderr` 之解碼、⛔ 任一被判之值**（本格之判只取 `returncode`）⇒ **本閘之綠成立**。🛑 **通則**：凡以 `text=True` 取含 CJK 路徑之 `git` 輸出者，**其 `stderr` 於 `cp950` 主控台可靜默丟失** ⇒ **判若繫於輸出字串**，須改**取 bytes**（`GB-88` 族·`常規五`） |

🔒 **六則自捕皆<u>於出艙前</u>攔下**、**⛔ 頂替**、**⛔ 鑄號**——鑄號**候發單側**。

---

## `§十`　🛑 本批之終結

> **`§三` 三-2 之 `停二` 成就 ⇒ 判器紅、停、上呈。**
> 工項二（`§四` 修批）**⛔ 辦**；KL 之放行**仍未動用**；`verify/W-G.9-299-gb170` **⛔ 開**；
> `GB-170` **⛔ 解除、⛔ 收窄**，其失效條件 `(2)`／`(3)` **仍未成就**。
> 🛑 **⛔ 呈 KL**；孰為 KL 射程 `(b)` 之「該處」、`停三` 之潛在成就（`wf_f4.py` 之 `*_EXPECT ＝ 2`）
> 與 `side_mid` 在域性不足之處置，**一律候發單側裁**。
