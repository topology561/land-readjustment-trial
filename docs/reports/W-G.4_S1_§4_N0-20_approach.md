...
## 0.5 ✅ KL 三裁定案（2026-07-20·reframing 已 KL＋claude.ai 雙獨立坐實）→ 本 approach 依此 REVISE

reframing 確立（claude.ai 亦逐條 grep 坐實）：F.1 死路·通式已在 F.4 E3·遞補錨閘恒綠·UC9898 終態 diff=0。**爆炸半徑前提作廢。**

**三裁（KL 鎖）**：
1. **甲**：R3/R6 末端整形**續在 F.4**（不搬 F.1·死路＋抵觸裁示1(b)）。土地影響 0。
2. **對**：§4 交付＝**在 F.4 E3 顯式形式化 R_end／末端帶構造 ＋ 無-winner fallback**。UC9898 終態不變。
3. **winner 判準＝乙**：末筆勝者宗需「**合格（臨正街 且 符畸零地寬·補丁七 §二.4）且 G≥area(R_end)**」方為 winner（整塊吞 R_end＝未臨正街∪末端帶）；否則（無合格宗·或合格但 G<area(R_end) 吞不下整個 R_end）→ **無 winner → 末筆抵費地嚴格＝area(R_end)**。
   > `G≥area(R_end)` 係「能否整塊吞下末端碎塊（含未臨正街）」測試·嚴於「符畸零地寬」（後者只蓋末端帶）·對應補丁七 §一.3「併入末筆勝者宗 或 隨末筆抵費地」之或結構。（覆寫前一輪「不加 G≥area」·KL 已更正。）

### 0.5.1 REVISE 落點＝F.4 E3 `_reshape_block`（wf_f4:1096·非 wf_f1）——碼面對映（已 recon）

現 `_reshape_block`（wf_f4:1096-1190·＝R1 機制通用化·對 R3/R6 生效）：
- `wedge = frag["poly"]`（:1153）＝**未臨正街**（s<0 碎片·R6 85.71）。
- **target 搜尋**（:1155-1161）＝`grp`（該側·累積S 序）首筆**合格**（`S>0.01 且 宗地寬度≥mw 且 not flagged`＝臨正街+畸零寬·跳失格）。
- **bisect**（:1178-1181）＝`union(strip(S),wedge).area==G_B` ⟺ strip=G_B−未臨正街（＝常數項 bisect）。

**§4 REVISE（三處·additive／改判準·**非搬波次**）**：
| # | 改 | 碼位 |
|---|---|---|
| 1 | **新 `_end_region_R(block,d̂,p1,cad_alloc,min_width)`**（模組級·ns·同 §3 `_corner_buffer_S` 先例）→ `(未臨正街, 末端帶, R_end, area(R_end))`。未臨正街＝block∩{s<0}（＝現 frag["poly"] 同物·可核對）；末端帶＝⊥ALLOCLINE 垂距帶 W=畸零寬（**§1 定案構造·B 盒由 block bounds 導·禁硬編**）；R_end＝∪。**side/端顯式參數化·缺 cad_alloc loud**。 | 新函式 |
| 2 | **winner 乙門檻**：target 搜尋後·加判 `G(target) ≥ area(R_end)`。合格 base 複用**現 target 搜尋**（wf_f4:1155-1161·**非** `_projection_order`·reviewer C-1）。 | wf_f4:~1162 |
| 3 | **無-winner fallback**：`target_row is None`（現 :1162 raise）**或** `G(target)<area(R_end)` → **不 raise**·改「**末筆抵費地嚴格＝area(R_end)**」（R_end 整塊成抵費地·未臨正街不單獨成筆）。**合成夾具**逼此分支（UC9898 無案例·latent）。 | wf_f4:1162-1163 |

**刪**：爆炸半徑段／「遞補錨閘證不越權」（`run_verification:707` 恒綠·讀 trunk-B·無效證據·**禁拿它證**）。

### 0.5.2 winner bisect（＝現 R1 逐字·不改）
