...
| 白縫舊閘 `<1㎡` | W-D.2:83 / W-D.1.3:201（**散文·無碼**·grep run_all/tests 空） | 無（W-C 殘餘 1.05＋餘裕） | 🔴 **散文閘·N0-17-b·本波立碼閘 ≤0.01** |
| `area >= 1.0` | 四病灶處 | 無（與守恆閘同常數 1.0·自洽盲區） | 🔴 **殘餘定閘·T1 廢·改 buffer(-1e-4).is_empty** |
...
| **守恆·全區 `<6.0`** | **run_verification:888/951/1009（真碼閘·CC 自驗）** | 無（43 萬元/次） | 🔴 **殘餘定閘·必立·收 6.0→0.01** |
| **守恆·逐街廓 `<1.0`** | **stepg:813/846·app.py:15341（真碼閘）** | 無（72,059 元/街廓） | 🔴 **殘餘定閘·必立·收 1.0→0.01** |
...
→ **⚠️ CC 自驗更正（上呈 claude.ai·見 `W-G.4_CC自驗_守恆閘存在性更正_上呈.md`）**：claude.ai 稱「守恆<6 不存在·散文閘」有誤——`run_verification:888/951/1009` 是**真碼閘**（全區守恆·`cons_resid<6.0`），非散文。真散文閘例＝**白縫 `<1㎡`**（grep run_all/tests/CLAUDE.md 空）。
→ 本波**必立三閘**（白縫/鋪滿/守恆·皆≤0.01·幾何精度上限）；守恆兩閘（全區 6.0→0.01·逐街廓 1.0→0.01）皆收緊。廢「area>=1.0」（T1）。`<5㎡`/`0.1` 暫標待歸因（不得再以全綠作正確性根據·N0-17）。
