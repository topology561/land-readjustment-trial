...
> 🔧 **段二-0 一-2 補正**：本檔初版引用該 log 時**該檔尚未入庫**（`git log --all -- <path>` 為空）
> ⇒ 屬「引用不存在之檔案」＝視同未驗。**已於段二-0 補入庫**。
> **其為產生輪之證據**（非推測）：
> ① mtime `2026-07-31 23:52:14` vs commit `56b345a` 之 `2026-07-31 23:56:55`（**早 4 分鐘**）；
> ② **已入庫**之 `docs/reports/W-G.5_K8-段三_commitC_重錨解封.md` §二 標題即
> 「驗收（`verify/out/K8_seg3_C.log`…）」⇒ 倉內既有文件已指認之。
> ⚠️ **並無其他候選**：全部**已入庫**之 `verify/out/*.log` 中，132 名目者皆為 **130 PASS／2 FAIL**
> （K-8 §三前之狀態），**無一支為 86／46** ⇒ 現行基線之產生輪**在本次補正前從未入庫**。
...
⇒ **PASS 86 支與 FAIL 46 支之名目集合逐字相同、零進零出。**
（🔧 段二-0 一-4 更正：本行原誤植為「PASS 46 支與 FAIL 86 支」，與上表 §5-1 之
PASS 86／FAIL 46 相反。**表為準、本行為誤**，已改。）
...
### 5-4 🔧 段二-0 一-3 補正：本輪之工作樹含未入庫之 harness 種子

本輪 `run_all` 係在**含 `data/地籍資料來源_匿名版.xlsx` 未入 commit 異動**之工作樹上跑的。
該檔**確為 harness 實際輸入**（`grep -n "ANON_XLSX = " verify/run_verification.py`
→ `grep -n "build_ownership(ns, fake_st, ANON_XLSX)" verify/run_verification.py`
→ `grep -n "def build_ownership" verify/selection_pipeline.py`），**非閒置檔**。

**已查明（非推測）**：

| 項 | HEAD 版 | 工作樹版 |
|---|---|---|
| 檔案大小 | 12,380,156 B | 11,482,185 B |
| zip 內部成員 | 10 | 12（多 `xl/sharedStrings.xml`／`docProps/custom.xml`）|
| `U_LAND` 工作表 | `(64453, 76)` | `(64453, 76)` |
| `重劃區地籍` 工作表 | `(53, 21)` | `(53, 21)` |

⇒ **兩工作表以 `pandas.testing.assert_frame_equal(..., check_exact=True)` 逐格比對·雙雙通過。**
差異**全在容器層**（字串改存 `sharedStrings` 而非 inline、theme／styles 置換）
——即**於 Excel 重存過一次**，**儲存格資料零變更**。
而 `build_ownership` 係以 `pd.read_excel` 取值，**只吃儲存格值、不吃樣式** ⇒
**對 132／86／46 零影響**。

⚠️ 另：該檔 mtime `2026-07-19 23:04` **早於**對照輪 `K8_seg3_C.log`（`2026-07-31 23:52`）
與本輪（`2026-08-03 14:04`）⇒ **兩輪吃的是同一份 xlsx**，故 §5-2 之集合比對**輸入一致**。

🚩 **該檔仍未 commit**（自 `1b290e4` 以來），維持上呈狀態——本批**不代 KL 決定是否入庫**。
