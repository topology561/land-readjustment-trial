...
🛑 本段之一切數**皆於報告 `commit`（`74cc1111802587cebb319637a99681f90856ee28`）之後實跑**；
🔒 **先跑再填**（`補令六 §一-1` 之機械順序）——本段落筆前無任何佔位以外之數。

### 一　倉內 `303` 單 `§六` 之九項（期值依 `補令六 §零-3` 之 `A4`／`A5` 取代）

| # | 產生指令 | 實得 | 期 | 判 |
|---|---|---|---|---|
| `1` | `git show --numstat --format= -z <各 commit>` | 逐檔具名見下；**三 `commit` 之刪除欄合計 ＝ `0`** | 全 `0` | ✅ |
| `2` | `git diff --name-only <前>..<後> \| grep -xE '<四檔>'`（逐區間） | `dbc5b1d4..b688ad10` ⇒ **`0`**／`b688ad10..094b8413` ⇒ **`0`**／`094b8413..74cc1111` ⇒ **`0`** 行 | 逐區間 `0` | ✅ |
| `2`造 | 同 pattern 於 `17fd981^..17fd981` | **`2`** 行 ＝ `['app.py', 'verify/stepg_pipeline.py']`（`git rev-parse -q --verify` `rc = 0`） | [必非零] | ✅ |
| `3` | `.md`／`.py` 之 `CR`（框 ＝ 倉內 blob·`git cat-file blob` **取 bytes**） | **`6`** 檔逐檔 `CR` ＝ **`0`**（二 `.md` ＋ 四 `.py`） | `0` | ✅ |
| `3`造 | `git cat-file blob HEAD:data/V6.dxf` | `CR` ＝ **`12308`**（全長 `88341` B） | [必非零] | ✅ |
| `4` | `python verify/probes/probe_WG9267_issuer_measurers.py registry` | 自誤 相異 **`400`**／MAX **`410`**（缺 `[106,355,356,357]`）｜`GB` `168`／`170`（缺 `[12,87]`）｜`VR` `80`／`95`（缺 `[73,75]`）｜`K-9` `28`／`29`（缺 `[]`）·`rc = 0` | `A4`·**零鑄號** | ✅ |
| `5` | `python verify/probes/probe_WG9269_pit_index.py` | **`90`／`90`**·末標籤 **`bl`** ⇒ 下一 **`bm`** | `A5`·⛔ 鑄坑 | ✅ |
| `5`附 | `git diff --numstat <基座> <HEAD> -- verify/probes/probe_WG9269_pit_index.py`（`ALPHA_LABELS` 之唯一 `.py` 載體） | 輸出 ＝ **空** | 空 | ✅ |
| `5`自洽 | 期初 `git ls-tree -r --name-only -z <基座>` ⋂ `*.md`（**⛔ `--with-tree`**·坑 `bl`） | 期初 **`815`**（**＝ `補令五` 收工閘 `5` 之期末·外部錨**）／期末 **`817`** ⇒ 增量 **`2`** ＝ 本批新增 `.md` 檔數 **`2`** | 自洽 | ✅ |
| `5'` | `python verify/tools/wg9223_acceptance_audit.py <本單> <本報告>` | `【驗收】` 段 ＝ `:212`–`:218`；**判定組 `30` 個**／參考組 `3` 個；**判定組之報告命中 `0` ＝ `0` 個**（參考組命中 `0` ＝ `1` 個·**⛔ 作判**·係單之行文片段）·`rc = 0` | 命中 `0` ＝ `0` | ✅ |
| `6` | 自限復驗（見下表） | 全數如期 | —— | ✅ |
...
| `commit` | 檔 | `+` | `-` |
|---|---|---|---|
| `b688ad10…` | `docs/orders/W-G.9-303_補令六_新窗首則與起手單.md` | `221` | `0` |
| `094b8413…` | `verify/out/WG9303R6_dynamic.log` | `1086` | `0` |
| `094b8413…` | `verify/out/WG9303R6_dynamic2.log` | `1091` | `0` |
| `094b8413…` | `verify/out/WG9303R6_static.log` | `236` | `0` |
| `094b8413…` | `verify/out/WG9303R6_static2.log` | `124` | `0` |
| `094b8413…` | `verify/probes/probe_WG9303R6_dynamic.py` | `210` | `0` |
| `094b8413…` | `verify/probes/probe_WG9303R6_dynamic2.py` | `224` | `0` |
| `094b8413…` | `verify/probes/probe_WG9303R6_static.py` | `425` | `0` |
| `094b8413…` | `verify/probes/probe_WG9303R6_static2.py` | `235` | `0` |
| `74cc1111…` | `docs/reports/W-G.9-303R6_補令六_工項一二_執行報告.md` | `488` | `0` |

#### 閘 `6` 自限復驗（逐項）

| 受詞 | 產生指令 | 實得 | 期 | 判 |
|---|---|---|---|---|
| `34` 檔 blob 期初＝期末 | 逐檔 `git rev-parse <基座>:<f>` vs `HEAD:<f>` | **相異 `0`** 檔 ⇒ `[]` | `0` | ✅ |
| 判別力[必相異] | 本批確有異動之檔 | **`10`** 個（非零 ⇒ 該檢⛔ 恆綠） | ≥ `1` | ✅ |
...
| `K-9-29`／`K-9-5-*`／`K-9-12` 子項 | 生產碼 `34` 檔相異 `0` | **⛔ 落地** | ⛔ 落地 | ✅ |
| `GB-79`／`167`／`168`／`169`／`170` | `GB` 簿 `168`／`170`（器之輸出）與開工閘逐格相同 | **⛔ 修** | ⛔ 修 | ✅ |
| 物證落檔逐位未動 | 先 `git cat-file -e HEAD:<p>` ⇒ `git rev-parse`（框 ＝ blob `sha1`） | **三態分列：態1 `13` ／ 態2 `0` ／ 態3 `0`** | 逐位未動 | ✅ |
| 本批新增之落檔 | 同上 | `['verify/out/WG9303R6_dynamic.log','…dynamic2.log','…static.log','…static2.log']`——其**期初不存在** ⇒ ⛔ 判為「未動」，係**新增** | —— | 照實 |

### 二　`補令六` 另加之二項

| # | 產生指令 | 實得 | 期 | 判 |
|---|---|---|---|---|
| `8-a` | 本波**七單**之在倉性：逐單 `git cat-file -e HEAD:<p>` ⇒ `git rev-parse HEAD:<p>`（**三態分列**·`rc` 緊接指令） | **七單皆態1**（`rc = 0`）：施工單 `8c85fc1ccdf781ce`／補令一 `87cb2813c3cf2040`／補令二 `3d91f357bb734c33`／補令三 `d03552c4bcfcef05`／補令四 `d709a39879e52d0e`／補令五 `db74056172eca9a6`／**補令六 `44a0ddaff4fd8c92`**；計 **`7`** | 皆態1 | ✅ |
| `8-a`造 | 一**執行期組出**之人造路徑（字面⛔ 出艙） | `git cat-file -e` ⇒ `rc = 128`（≠ `0`） | [必為零] | ✅ |
| `8-b` | **零鑄號之性質**（以**器之輸出**量·⛔ 以「未鑄」之宣稱充之） | 自誤 相異 **`400`**／MAX **`410`**·`GB` `168`／`170`·`VR` `80`／`95`·`K-9` `28`／`29`·坑 **`90`／`90`** 末 **`bl`** ⇒ **與開工閘之值逐格相同**；`ALPHA_LABELS` 之 `git diff --numstat` 輸出**空** | 逐格相同 | ✅ |

🔒 **六單之 blob 與 `補令六 §零-1` 所載逐位相符**（`8c85fc1c`／`87cb2813`／`3d91f357`／`d03552c4`／`d709a398`／`db740561`）。

### 三　🩸 **自捕 `5`**（本段新增·⛔ 追改上文之「自捕 `4` 則」一字 ⇒ **本批自捕實為 `5` 則**）

| 項 | 內容 |
|---|---|
| 形 | 🔴 **四份 `.log` 落檔為 CRLF** |
| 徵候 | 倉內 blob 之 `CR` ＝ `1086`／`1091`／`236`／`124`（**恰等於各自之 `LF`** ⇒ 全檔 CRLF） |
| 成因 | 落檔係以 **shell 重導向 Python `stdout`**（`> file`）為之，Windows 下 `stdout` 為**文字模式** ⇒ `\n` 譯為 `\r\n`。**違恆常附款**「**落檔一律以 binary 模式寫**（`open(p,'wb')`·⛔ `io.open(p,'w')`）」 |
| 與閘 `3` 之關係 | 🛑 **閘 `3` 未紅**——其受詞逐字係 **`.md`／`.py`**，該 `6` 檔之 `CR` 皆 `0`。⛔ 以「閘未紅」充「無誤」 |
| 處置 | 🛑 **本批⛔ 修**，理由**二者皆與上位禁令互斥**：(a) 以 `--amend` 修（＝ `補令四` 自捕 `3` 之既例）須 **force-push** 已推之 `094b841320b461cdc91f6ce715a492d86c55aa64` ⇒ ⛔；(b) 另開 `commit` 改寫該四檔 ⇒ `deletions > 0` ⇒ **反破閘 `1`**。⇒ **照實登記、處置候發單側**（⛔ 鑄號·候 `W-G.9-304`） |
| 攔下它的 | **收工閘 `3` 之逐檔出艙**——若僅報「`.md`／`.py` 皆 `0`」而不逐檔列全量，本則即不可見 |

🛑 **⛔ 讀為「本批無誤」**——本批自捕合計 **`5`** 則，**全為 CC 自身量測器／落檔之缺陷**。

### 四　`⛔ 為之事` 之補充自檢（本段所涉）

| 受詞 | 判 | 證 |
|---|---|---|
| ⛔ 於任何入倉文件宣稱一件尚未發生之事 | ✅ **未為** | 本段之閘 `7`（`push` 後之 `ls-remote`）**⛔ 入倉**、只出艙於回報 |
| ⛔ 追改任何已入倉文件一字 | ✅ **未為** | 本段係**嚴格末端追加**；`§五` 之前一字未動（`deletions` 見末表） |
| ⛔ 鑄任何號 | ✅ **未為** | 閘 `4`／`5`／`8-b` 之器輸出與開工閘**逐格相同** |

### 五　逐 `commit`（**全 `40` 碼**·本段補入第 `3`／`4` 列）

| # | `commit` | 內容 | `deletions` |
|---|---|---|---|
| `1` | `b688ad10e56054274c501b0d244b3c6cb196161a` | `§二` 工項零：本補令原封入倉 | `0` |
| `2` | `094b841320b461cdc91f6ce715a492d86c55aa64` | `§三`／`§四` 之四支量測器與四份落檔 | `0` |
| `3` | `74cc1111802587cebb319637a99681f90856ee28` | 執行報告入倉 | `0` |
| `4` | （本段之 `commit`·收工閘末端追加） | 收工閘之實測 | `0` |
