...
## 1.5 🎯 R_end 幾何錨 REPRODUCED（KL 指定·純幾何獨立·禁假設·對錨後定案構造）

**R6 錨（plan §4-1）＝85.71 未臨正街 ＋ 166.57 末端帶 ＝ 252.28** —— **逐位重現**（`docs/reports/probes/probe_§4_R_end.py`·純 CAD 座標·∥ALLOC 斜交；⚠️ 此表 R3「0.0001/153.77」係**左端錯算**·R3 右側案真碎片 78.24·見 D-右側 上呈）：

| 塊 | 未臨正街（s<0）| 末端帶（⊥ALLOCLINE·W=畸零寬 3.5）| R_end ∪ |
|---|---|---|---|
| **R6** | **85.7064** | **166.5696** | **252.2760**（錨 252.28·**Δ=−0.004**·座標精度級 ✅）|
| R3 | 0.0001 | 153.7720 | 153.7721 |
| R1 | 5.3255（＝`WEDGE_AREA_ANCHOR` 5.30）| 109.8135 | 115.1390 |

**🔴 對錨過程抓出 CC 構造錯（KL「對不上→停·重推」正中）**：
- **末端帶 ≠ s-strip**：初擬 `_block_strip(p1, S=畸零寬)`（s 軸推進）得 **166.03**·差錨 −0.54㎡。
- **re-derive 定案**：R6 實為**斜交**（`d̂·ALLOCLINE = 85.391°`·非 plan §4-1 所稱「θ=90° 特例」）→ **末端帶＝⊥ALLOCLINE 平移帶**（兩線 ∥cad_alloc·**垂距**＝畸零寬）＝**166.5696 逐位命中錨**（＝補丁七 §二.2「自 mp 往街廓內量 W」之**垂距**義·非 s 軸）。s-strip 差 1/cos(4.6°)=1.0032 倍。
- **p1-pz FRONTLINE 弦（作廢·補丁七 §二.5）**＝166.03（同 s-strip）——plan §4-1「θ=90° 弦長對得上」係 R6 非嚴格 90° 下之近似誤述；**垂距構造（166.57）方為通式**。

**定案 R_end 構造（approach 用·禁本案常數）**：
- **未臨正街** ＝ `block ∩ {s < 0}`（末端 ALLOCLINE 半平面外側·s=0 線＝過 p1 之 ∥cad_alloc 線·＝補丁七 §一.1「半平面」）·經 `_strip_s_range`（∥ALLOC 斜交）。
- **末端帶** ＝ `block ∩ (⊥ALLOCLINE 平移帶·垂距 = 畸零寬〔查表〕)`（過 p1·法向 rot90(cad_alloc) 往街廓內定號）。**禁 s-strip**（斜交下差一個 1/cos）。
- **R_end ＝ 未臨正街 ∪ 末端帶**（s<0 與 s>0 側·共 s=0 界·不疊·union=sum）。
- 全走該塊 `cad_alloc`／`front_lines`·畸零寬查表·**零本案常數**（同 §3 泛用四約束）。
...
- **W-D.4 遞補錨閘**（**權威＝`run_verification:707`·非 wd4:316 註**）錨定：**R6 85.66→`628-4(1)`（v3 重錨·跳過 `628(2);628-1(2);628-23(1)`）**。⚠️ **KL 2026-07-20 更正**：v2 標的 `628-1(2)` 於 v3 **自身入畸零**（寬<3.5m）→ 遞補規則（沿分配序往前找第一筆 S≠0 且 寬≥min_width 之有效宗）**續往內移到 `628-4(1)`**；`wd4:316` 舊註「→628-1(2)·跳過 628(2)」係 **v2 過期標的**（#26a·本 recon 前版誤抄·已同 push 更正）。F.4 E3 終態遞補整形（`wf_f4:682`·純幾何·池不變）。
- **改通式後**：R3/R6 於 F.1 即以 R_end 實幾何被勝者吃下 → **F.4 遞補錨之標的可能變**（碎片不再殘留至 F.4）→ 須跑 **W-D.4 遞補錨回歸閘**證**不越權**（plan §4/§10 既有機制·#24-1「正典條文只在其落點適用·越權爆炸半徑大一數量級」）。**⚠️ 送 reviewer 實測爆炸半徑時·標的一律用 v3 實況 `628-4(1)`·禁用過期 `628-1(2)`（KL 叮）。**
