...
| **GB-8** 🆕 | **per-block MinA 有兩份實作、無一致性看守**——`wf_f0._mina_by_block`（3 參數→`dict`·**土地後果路徑**：`wf_f0`/`wf_f2`/`wf_f3`/`wf_f4` 與 harness 皆用之）與 `wd4_tier_list._mina_by_block`（4 參數→`(dict, min)`·**清單／回歸路徑**）。公式今日相同，但**全倉無任何閘比對二者** | 任一支被改（去錨／K-6-A2 拆除達標角色／換 round 位置）而另一支未同步 ⇒ **靜默分歧**：清單與回歸斷言仍綠、土地後果已變（或反之） | **不改**（本波只登記）。二支簽章不同**不可互代**，故非「刪一支」即可了事 | `grep -rn "_mina_by_block" --include="*.py" .`；一致性看守之不存在：同一 grep 加 `\| grep -i "assert\|==\|一致\|同源"` ⇒ **僅命中註解、無斷言** | ❌ **從未被看守**（今日相同係二者碼面巧合一致，非機器保證） | ❌ 不需——屬 #20「避第三份抄寫」之未竟部分。**K-6-A2 拆除 per-block MinA 達標角色時一併收** |
