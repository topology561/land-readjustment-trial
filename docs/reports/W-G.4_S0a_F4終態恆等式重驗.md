...
- 0m F.4 之 E2 7-5 批次窮舉「可行 1929/3888」（遠多於 3.5m 之 240）→ 完整 f0→f4 跑 sgE 之數值分解受**效能限**（單跑 >15min·background 屢遭殺）。
- **已坐實（前 S0 報告 8ceacc2 · wg4_s0_repro）**：0m net(CSV) 12/12 單向正（R1+0.17/R2+0.69/R3+0.05/R4+0.12/R5+0.26/R6+0.17）——**型態同 3.5m**。
- **機制論證（讀碼·§一）**：0m 之 g_tab 亦 `build_step_g_tables(sgE)`（同 code）·E3 亦 `pool_final=poolE`（同 code）→ 0m 之 F.1/E3 同樣不進 g_tab 池·恆等式機制與 3.5m 完全相同。
- 0m 完整數值分解 background 續跑中（若完成補入本報告 §三-數值）。**非假設**：0m net 型態已坐實·機制讀碼同源·僅完整分解表待效能允許。
